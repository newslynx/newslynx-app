...
var exportHelpers = {
			...
			articlesAwaitAll(function(err, articlesList){
				...
				var rows = articlesList.map(function(articleInfo){
					...
					// _.each(csv_schemas, function(schema, csvName){
						...
						// csvs[csvName] = [];
						// var articleInfo = csv_data_sources[csvName]
					var row = {}
	...
	timeseries: function(timeseriesAwaitAll, now, cb) {
		timeseriesAwaitAll(function(err, timeseriesLists){
			timeseriesLists.forEach(function(articleTimeseries){
				articleTimeseries = articleTimeseries.map(exportHelpers.addExportedDate(now));
			})
			cb(err, timeseriesLists)
		})
	...
	events: function(eventAwaitAll, now, delimiters, uniqIdList, cb){
		...
		eventAwaitAll(function(err, eventsInfos){
			...
			var events_by_id = exportHelpers.segregateById(eventsInfos[0].events, uniqIdList)
			events_by_id.forEach(function(eventsForId, index){
				eventsForId = flattenEvent(eventsForId, uniqIdList[index]).map(exportHelpers.addExportedDate(now));
			})
			cb(err, events_by_id);
		})
