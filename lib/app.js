...
app.set('trust proxy', ['loopback', 'linklocal', 'uniquelocal', '127.0.0.1']) // specify multiple subnets as an array
...
app.use(function(req, res, next){
	...
	if ( non_auth_routes.indexOf(page) !== -1 ) {
		...
		if (req.session.apikey) {
			...
			req.session.redirect_page = url_path
			// Persist this request after login for bookmarklet route
			if (url_path == '/articles/lookup'){
				url = req.query.url;
				req.session.redirect_url = url_path + '?url=' + url;
			}
			res.redirect('/login');
		}
