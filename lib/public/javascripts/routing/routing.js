routing = {
	Router: Backbone.Router.extend({
		...
		readPage: function(mode, ids){
			...
			if (ids.exiting) app.helpers.drawer.addRemove.call(app.instance, mode, ids.exiting, false);
			if (ids.entering) app.helpers.drawer.addRemove.call(app.instance, mode, ids.entering, true);
			if (!ids.exiting && !ids.exiting) app.helpers.drawer.setModeOnly(mode);
		},
		loadAllInSection: function(mode){
			console.log('loading all in section')
			app.helpers.drawer.setModeOnly(mode);
			app.helpers.drawer.enableAll.call(app.instance);
		},
		set: {
