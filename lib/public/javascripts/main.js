			...
			set: {
				...
				compare: function(articleUid, isNew){
					...
					if ( isNew ){
						...
						if ( routing.helpers.hasArticleUid(hash) ) hash += '&';
						// Append current selections to hash
						hash += (articleUid);
					} else {
						// Remove the id from the hash and any duplicate ampersands
						hash = hash.replace(new RegExp('(&|)'+articleUid, 'g'), '').replace(/&&/g, '&');
					}
	...
	var views = {
		...
		App: Backbone.View.extend({
			...
			initialize: function(){
				...
				this.listenTo(collections.article_summaries.instance, 'change:viewing', this.mainArticle.update);
				...
				this.listenTo(collections.article_summaries.instance, 'change:viewing', this.makeComparison);
			...
			mainArticle: {
				update: function(articleModel){
					var is_new = this.mainArticle.check(articleModel),
							article_uid = articleModel.get('article_uid');
					// If it is a `viewing` article, fetch its data
					if (is_new) this.mainArticle.fetch( article_uid );
					return this;
				},
				check: function(articleModel){
					...
					return articleModel.get('viewing');
				},
				...
				bake: function(detailModel){
					...
					this.$content.html(article_detail_view.render().el);
					return this;
				}
			},
			divisionSwitcher: {
				update: function(model){
					// views.app.divisionSwitcher.setDomState(model);
					views.app.divisionSwitcher.setHashState.call(this, model);
					return this;
				},
				setHashState: function(model){
		...
		ArticleSummary: Backbone.View.extend({
			...
			style: {
				toggleActive: function(articleModel){
					console.log(articleModel.get('viewing'))
