	...
	window.app = {
		...
		Articles: Backbone.View.extend({
			...
			initialize: function(){
				...
				this.compare.detailData = pageData.articleSummaries;
		...
		helpers: {
			...
			itemDetail: {
				...
				fetch: function(itemUid, action, cb){
					...
					var mode = models.section_mode.instance.get('mode');
					var loaded_matches = _.filter(this[mode].detailData, function(obj) { return obj[app.instance.listUid] === itemUid });
	...
	var views = {
		...
		ArticleDetailRow: Backbone.View.extend({
			...
			render: function() {
				...
				$el.attr('data-qual-subject-tags', this.data.qual_metrics['subject-tags'].length);

				_.each(this.data.qual_metrics['impact-tags'], function(category){
			...
			transformData: function(modelData){
				modelData.qual_metrics = d3.nest()
					...
					.rollup(function(list){
						...
						return list.map(function(d) { return pageData.org[d.metric.replace('-','_')].filter(function(f){ return f.uid == d.id })[0] });
					})
					.map(modelData.qual_metrics);

				// Next by tag category
				modelData.qual_metrics['impact-tags'] = d3.nest()
					...
					.entries(modelData.qual_metrics['impact-tags'])
