		...
		helpers: {
			...
			toUserTimezone: function(utcDate){
				var utc_date = new Date(utcDate),
						user_timezone_date = new Date(utcDate).setHours(utc_date.getHours() + parseFloat(pageData.org.user.timezone) );
				
				return userDateObjectInTimeZone;
			},
				...
				this.spottedTail = spottedTail()
					.x(function(d) { 
						var utc_date = that.formatDate.parse(d.datetime),
								user_timezone_date = utc_date.setHours(utc_date.getHours() + parseFloat(pageData.org.user.timezone) );
