(function(){
		...
		Articles: Backbone.View.extend({
			...
			articleSummaryList: {
				...
				changeActive: function(mode, uids){
					this.setState(mode);
					collections.article_summaries.instance.setTrueByIds('viewing', 'article_uid', uids);
				},
				filter: function(){
	...
	var listeners = {
		...
		// 	$('#main-wrapper').on('click', '.active a', this.actions.killEvt);
		// 	// And subnav options, which are a tags for semantics and style consistency
		// 	$('.subnav-option a').on('click', this.actions.killEvt);
		// },
		// actions: {
		// 	killEvt: function(e){
		// 		e.preventDefault();
