	...
	mainData.alertCreators = [
		{
			...
			schema: {
				...
				result_type: {
					type: "select",
					options: ["option1","everything","option3"]
				},
				language: {
					type: "select",
					options: ["option1","option2","english"]
				},
				region: {
					type: "select",
					options: ["option1","any","option3"]
				},
				how_many: {
					type: "select",
					options: ["option1","best","option3"]
				}
			}
		},{
			service: "Twitter",
		...
		helpers: {
			...
			formJsonToMarkup: {
				text: function(service, prettyName, data){
					...
					markup += '<label for="'+name_id+'"> '+prettyName+'</label> <input type="text" name="'+name_id+'" id="'+name_id+'" value="'+value+'"/>';
					markup += '</div>';
					return markup;
				},
			...
			bakeForm: function(service, schema, help){
				var form = '';
