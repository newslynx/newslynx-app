	...
	var app = {
		...
		Settings: Backbone.View.extend({
			...
			getSettingsData: function(){
				var settings = {};
				this.$el.find('.inputs-container').each(function(){
					var $this = $(this),
					...
					if ( !$inputs.hasClass('multi') ){
						...
						input_val = $inputs.find('input').val();
						if ( input_val.trim() ) {
	...
	var views = {
		...
		ArticleDetailRow: Backbone.View.extend({
			...
			update: function(){
				...
				var cells = this._el.data([data]).enter(); // This should work with `datum` and not have to wrapp in an array, but that is giving an undefined enter selection.
			...
			goToDetail: function(e){
				...
				app.instance.divisionSwitcher.updateHash('single');
				routing.router.set['radio'](article_uid);
			},
