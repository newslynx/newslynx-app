(function(){
	...
	mainData.river_items = [
		...
		},
		{
			"alert_uid": "a4",
			"status": "pending",
			"title": "ProPalpatine exposes anti-fracking movement's ties to the Rebels",
			"text": "The carbon in our apple pies billions upon billions cosmos. Extraplanetary Hypatia. Tendrils of gossamer clouds? Rogue stirred by starlight across the centuries cosmic ocean white dwarf billions."
		}
	]

	mainData.alertCreators = [
		{
			service: "Google",
			help: {
				hint: "TKTK"
			...
			service: "Twitter",
			help: {
				link: "https://dev.twitter.com/docs/using-search"
			},
			schema: {
				search_query: {
					type: "text"
			...
			service: "Reddit",
			help: {
				link: "http://www.reddit.com/wiki/search",
				hint: "Hint: site:propalpatine.org AND search-term"
			},
			schema: {
				search_query: {
					type: "text"
	...
	var templates = {
		...
		helpers: {
			...
			bakeForm: function(uid, settings){
				var that = this,
						markup = '';
				// TODO, have this run off of the json structures defined in `mainData.alertCreators`
				_.each(settings, function(entry, field){
					markup += '<div class="entry">';
					markup += '<label for="'+field+'-'+uid+'">'+that.prettyName(field)+'</label>'
					markup += '<input id="'+field+'-'+uid+'" name="'+field+'-'+uid+'" type="text" value="'+that.escapeQuotes(entry)+'"/>'
					markup += '</div>'
				});
				return markup;
			},
			formJsonToMarkup: {
				text: function(service, prettyName, data){
					...
					return '<label for="'+name_id+'"> '+prettyName+'</label> <input type="text" name="'+name_id+'" id="'+name_id+'" />'
				},
			...
			bakeFormCreator: function(service, schema, help){
				var form = '';
				_.each(schema, function(fieldData, fieldName){
					var pretty_name = this.prettyName(fieldName);
					form += this.formJsonToMarkup[fieldData.type](service, pretty_name, fieldData);
				}, this);
				form += '<div class="buttons-container"><button class="cancel">Cancel</button><input class="save" type="submit" value="Create"/>'
				return form;
			},
	...
	var collections = {
		init: {
			...
			"approval-river": function(){
				...
				// Alert creators
				this.drawer_items.instance_static = new this.drawer_items.Collection(mainData.alertCreators);
	...
	var routing = {
		init: {
			...
			"approval-river": function(){
				...
				this.starting_route = 'my-alerts/'+app.helpers.drawer.getAllUids.call(app.instance);
			}
		},
	...
	var app = {
		...
		Articles: Backbone.View.extend({
			...
			divisionSwitcher: {
				...
				updateHash: function(entering_mode){
					...
					var new_uids = '';
					// var new_uids = models.section_mode.instance.get('previous-uids') || '';
		...
		ApprovalRiver: Backbone.View.extend({
			...
			initialize: function(){
				...
				// This is the array of objects we'll filter by id in order to get the crossover data from our summary items
				this.drawerData = mainData.alerts;
