(function(){
	...
	var templates = {
		...
		helpers: {
			...
			conciseDate: function(isoDate){
				var full_date_string = new Date(isoDate).toISOString(), // "2014-06-24" -> "2014-06-24T00:00:00.000Z, to this to standardize
			...
			bakeRecipeUpdateForm: function(uid, service, settings){
				var that = this,
						markup = '',
						schema = {},
						schema_with_selects = {};

				$.extend(true, schema, _.filter(pageData.recipeSchemas, function(recipeSchema){ return recipeSchema.service == service })[0].schema);
	...
	var collections = {
		init: {
			articles: function(){
				// Tags
				this.subject_tags.instance = new this.subject_tags.Collection(pageData.org.tags);
				this.impact_tags.instance = new this.impact_tags.Collection(pageData.org.tags);
				// Article summaries
	...
	window.app = {
		...
		Articles: Backbone.View.extend({
			...
			articleDetail: {
				prepTheDom: function(sectionModel){
					// TODO, maybe move this into a jade template on load and not have it load through underscore
		...
		ApprovalRiver: Backbone.View.extend({
			...
			scrollTo: function(e){
				...
				this.$content.animate({
					scrollTop: $('#'+dest+'-recipe').offset().top
				}, 200);
			},
		...
		Settings: Backbone.View.extend({
					...
					if ( !$inputs.hasClass('multi') ){
						...
						$inputs.each(function(){
							...
							$(this).find('.input-item').each(function(){
								...
								// If it has a key then insert an object
								// Otherwise insert the value into the array
								if (input_key) {
			...
			validateSettings: function(settings){
				var msg = true;
				if (settings.password[0] != settings.password[1]) msg = this.reportError('Passwords do not match');
				return msg;
			}
	...
	var views = {
		...
		ArticleDetailGrid: Backbone.View.extend({
			...
			sortColumn: function(e){
				...
				app.instance.$isotopeCntnr.isotope({ sortBy : dimension, sortAscending: this.sortAscending })
			}
		}),
		ArticleDetailRow: Backbone.View.extend({
			...
			render: function() {
				...
				this.$el.attr('data-title', model_json.title)
								.attr('data-date', model_json.pub_datetime);

				// Add a whole bunch of quant attributes dynamically
				_.each(model_json.bullets, function(bullet){
					$el.attr('data-'+bullet.dimension, bullet.count);
				})
				// Add aggregate values for bars
				_.each(model_json.bars, function(bar){
					var agg_count = 0;
					_.each(bar.counts, function(eventOrTag){
						agg_count += eventOrTag.count;
					});
					$el.attr('data-'+bar.dimension, agg_count);
				});
			...
			update: function(cb){
				...
				var row = this._el.data([data]).enter(); // This should work with `datum` and not have to wrap in an array, but that is giving an undefined enter selection.
				...
				row.append('div')
						...
						.classed('date', true)
						.classed('single', true)
						.attr('data-date', function(d) { return d.pub_datetime } )
						.html(function(d) { return templates.helpers.conciseDate(d.pub_datetime) });

				// Make a container for the bullet
				var bullet_container = this._el.data(data.bullets).enter()
					.append('div')
						.classed('cell', true)
						.classed('multi', true)
						.classed('gfx', true)
				...
				// Make a container for bars
				var bar_container = this._el.data(data.bars).enter()
					.append('div')
						.classed('cell', true)
						.classed('bars', true)
						.classed('gfx', true)
						.append('div')
							...
							.attr('data-group', function(d){ return d.dimension });


				bar_container.each(function(d){
					var _this = d3.select(this)
					// Add a column group for the bars to go into

					d.counts.forEach(function(citationSource){
						var _bar_column = _this.append('div')
							.classed('bar-column', true)
							.attr('data-name', citationSource.name)

						if (+citationSource.count > 4) {
							// Add the label
							_bar_column.append('div')
								.classed('bar-label', true)
								.html(citationSource.count)

							_bar_column.append('div')
									.classed('bar', true)
									.style('background-color', function() { return citationSource.color });

						} else {
							_bar_column.selectAll('.bar').data(_.range(citationSource.count)).enter()
								.append('div')
									.classed('bar', true)
									.style('background-color', function() { return citationSource.color });
						}
