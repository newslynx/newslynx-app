	...
	var routing = {
		...
		helpers: {
			diffUids: function(mode, newUids){
				...
				if (!this.history.length || !this.history[this.history.length - 1].uids) { obj.entering = newUids; return obj; }
	...
	var views = {
		...
		ArticleDetailRow: Backbone.View.extend({
			...
			update: function(cb){
				...
				var cells = this._el.data([data]).enter(); // This should work with `datum` and not have to wrap in an array, but that is giving an undefined enter selection.
