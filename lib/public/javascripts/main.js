(function(){
	...
	var collections = {
		init: {
			articles: function(){
				// Tags
				this.article_tags.instance = new this.article_tags.Collection(pageData.org.article_tags);
	...
	var app = {
		...
		Articles: Backbone.View.extend({
			...
			initialize: function(){
				...
				this.$articleTagList = $('.option-container[data-type="article-tags"] .tag-list');
				...
				this.listenTo(collections.article_tags.instance, 'change:active', this.drawer.filter);
			...
			bake: function(){
				// Article tags
				collections.article_tags.instance.each(function(tag){
					var tag_view = new views.Tag({ model: tag });
					this.$articleTagList.append(tag_view.render().el);
				}, this);
				// Impact tags
		...
		ApprovalRiver: Backbone.View.extend({
			...
			scrollTo: function(e){
				...
				this.$content.animate({
					scrollTop: $('#'+dest+'-alert').offset().top
				}, 200);
			},
