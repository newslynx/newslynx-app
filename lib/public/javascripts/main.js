		...
		init: {
			articles: function(){
				...
				articles.forEach(function(article){
					var section_model = new this.article_summary.Model(article);
					this.article_summary.instances.push(section_model);
				}, this);

				// Keep track of whether we're in single view or comparison view
				this.section_mode.instance = new this.section_mode.Model();
				this.section_mode.instance.set('mode', 'single')
				// // A model for the full article view
				this.article_detail.instance = new this.article_detail.Model();
			},
			"approval-river": function(){
				// Keep track of whether we're in `my-alerts or 'create-alert' view
				this.section_mode.instance = new this.section_mode.Model();
				this.section_mode.instance.set('mode', 'my-alerts');
			}
		},
		// Common
		section_mode: {
	...
	var routing = {
		...
		Router: Backbone.Router.extend({
			...
			stripTrailingSlash: function(mode){
				app.instance.articleSummaryList.setMode(mode);
				routing.router.navigate(mode, {replace: true});
			},
			readArticle: function(mode, uids){
				app.instance.articleSummaryList.changeActive(mode, uids);
			},
			set: {
	...
	var app = {
		...
		Articles: Backbone.View.extend({
			...
			initialize: function(){
				...
				this.listenTo(collections.tags.instance, 'change:active', this.articleSummaryList.filter);
			...
			divisionSwitcher: {
				...
				updateHash: function(sectionMode){
					...
					app.instance.articleSummaryList.changeActive(mode, new_uids);
			...
			articleSummaryList: {
				...
				changeActive: function(mode, uids){
					this.setMode(mode);
					if (uids) collections.article_summaries.instance.setTrueByIds('viewing', 'article_uid', uids);
				},
				filter: function(){
