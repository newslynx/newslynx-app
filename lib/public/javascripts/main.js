(function(){
	...
	var templates = {
		...
		articleDetailCardFactory: _.template( $('#article-detail-card-templ').html() ),
		helpers: {
	...
	var routing = {
		...
		Router: Backbone.Router.extend({
			routes: {
				'single/:uid': 'readSingle',
				'compare/:uids': 'readCompare',
				'single(/)': 'setSingle',
				'compare(/)': 'setCompare'
			},
			setSingle: function(){
				this.setState('single');
				routing.router.navigate('single', {replace: true});
			},
			setCompare: function(){
				this.setState('compare');
				routing.router.navigate('compare', {replace: true});
			},
	...
	var views = {
		Tag: Backbone.View.extend({
			...
			render: function(){
				var tag_markup = this.template( _.extend(this.model.toJSON(), templates.helpers) );
		...
		ArticleSummary: Backbone.View.extend({
			...
			initialize: function(){
				...
				// this.listenTo(this.model, 'change:viewing', this.set.detailView);
				// this.listenTo(models.article_mode.instance, 'change:mode', this.setViewingFromList);

			},

			render: function(){
				var article_summary_markup = this.template( _.extend(this.model.toJSON(), templates.helpers) );
			...
			set: {
				...
				// detailView: function(articleModel){
				// 	// var article_detail_model = collections.article_details.instance.where({article_uid: articleModel.get('article_uid')})[0];
				// 	// Show its detailed view
				// 	if ( articleModel.get('viewing') ){
				// 		// article_detail_model.set('viewing', true);
				// 	} else {
				// 		// article_detail_model.set('viewing', false);
				// 	}
				// }
			},
			...
			setViewingFromList: function(){
				// Zero all out
				collections.article_summaries.instance.zeroOut('viewing');
				// Grab our previous uids to now set to view
				var uids_to_view = models.article_mode.instance.get('previous-uids');
				uids_to_view.forEach(function(uid){
					collections.article_summaries.instance.where({article_uid: uid})[0].set('viewing', true);
				})
			}
		}),
		ArticleDetail: Backbone.View.extend({
			...
			render: function(){
				var article_detail_markup = this.template( _.extend(this.model.toJSON(), templates.helpers) );
		...
		ArticleDetailCard: Backbone.View.extend({
			...
			template: templates.articleDetailCardFactory,
		...
		DivisionSwitcher: Backbone.View.extend({
			...
			updateHash: function(){
				...
				if ( routing.helpers.hasArticleUid(window.location.hash) ){
					...
					new_uids = this.model.get('previous-uids') || current_uids;
					...
					if (mode == 'single' && current_uids.length > 2) new_uids = [current_uids[0]];
				}

				console.log(current_uids, new_uids)
