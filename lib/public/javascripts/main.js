(function(){
	...
	var templates = {
		init: {
			articles: function(){
				...
				this.DrawerListItem = _.template( $('#article-summary-templ').html() );
			...
			"approval-river": function(){
				this.DrawerListItem = _.template( $('#alert-templ').html() );
			}
		},
		helpers: {
	...
	var app = {
		...
		Articles: Backbone.View.extend({
			...
			initialize: function(){
				...
				this.listenTo(collections.drawer_items.instance, 'change:viewing', app.helpers.itemDetail.set.go);
				...
				this.listenTo(collections.detail_items.instance, 'add', app.helpers.itemDetail.listen.bake);
				this.listenTo(collections.detail_items.instance, 'remove', app.helpers.itemDetail.listen.destroy);
			...
			drawer: {
				...
				changeActive: function(mode, uids){
					this.setMode(mode);
					if (uids) collections.drawer_items.instance.setTrueByIds('viewing', 'uid', uids);
				},
				filter: function(){
			...
			itemDetail: {
				set: {
					go: function(articleSummaryModel){
						var is_new = app.helpers.itemDetail.set.check(articleSummaryModel),
								uid = articleSummaryModel.get('uid'),
								destination;
						// If it is a `viewing` article, fetch its data
						if (is_new) {
							destination = app.helpers.itemDetail.set.add;
						} else {
							destination = app.helpers.itemDetail.set.remove;
						}
						app.helpers.itemDetail.set.fetch( uid, destination );
						return this;
					},
					...
					fetch: function(itemUid, cb){
						...
						var loaded_matches = _.filter(articles_detailed, function(obj) { return obj.uid === itemUid });
					...
					add: function(itemData){
						collections.detail_items.instance.add(itemData);
					},
					remove: function(itemData){
						var uid = itemData.uid;
						collections.detail_items.instance.remove( collections.detail_items.instance.where({uid: uid})[0] );
					}
				},
				listen: {
	...
	var views = {
		...
		DrawerListItem: Backbone.View.extend({
			...
			render: function(){
				var drawer_list_item_markup = templates.DrawerListItem( _.extend(this.model.toJSON(), templates.helpers) );
			...
			radioSet: function(){
				// This isn't a toggle, so only fire if what we just clicked was not active
			...
			setViewing: function(){
				...
				routing.router.set[behavior]( this.model.get('uid'), this.model.get('viewing') );
				return this;
			}
		}),
