(function(){
	...
	mainData.alerts = [
		{
			...
			"service": "NewsLynx",
			"settings": {
				"name": "Trackbacks on other news sites",
		...
		},
		{
			...
			"service": "Reddit",
			"settings": {
				"name": "Domain mentions on Reddit",
				"search_query": "propalpatine.org"
			}
		},
		{
	...
	var templates = {
		...
		helpers: {
			...
			getAssociatedItems: function(uid, itemKey, itemsObj){
				itemsObj = mainData[itemsObj];
				return _.filter(itemsObj, function(obj) { return obj[itemKey] == uid });
			},
			countAssociatedItems: function(uid, itemKey, itemsObj){
				var associated_items = this.getAssociatedItems(uid, itemKey, itemsObj);
				return associated_items.length;
			},
			bakeForm: function(settings){
				var markup = '';
				var that = this;
				_.each(settings, function(entry, field){
					markup += '<div class="entry">';
					markup += '<label>'+that.prettyName(field)+'</label>'
					markup += '<input type="text" value="'+that.escapeQuotes(entry)+'"/>'
					markup += '</div>'
				});
				return markup;
			},
	...
	var collections = {
		init: {
			...
			"approval-river": function(){
				// Alerts
				this.drawer_items.instance = new this.drawer_items.Collection(mainData.alerts);
	...
	var app = {
		...
		ApprovalRiver: Backbone.View.extend({
			...
			initialize: function(){
				...
				// This is the array of objects we'll filter by id in order to get the crossover data from our summary items
				this.detailData = mainData.river_items;
			...
			river: {
				...
				resetViewAllBtn: function(){
					...
					var active_filters = collections.drawer_items.instance.where({viewing: true}).length,
							filters_enabled = active_filters > 0;
					this.$viewAll.toggleClass('active', filters_enabled);
				}
			}
		}),
	...
	var views = {
		...
		DrawerListItem: Backbone.View.extend({
			...
			events: {
				'click .drawer-list-outer': 'setModel',
				'click .settings-switch': 'toggleTray',
				'click .master-switch': 'toggleEnabled'

			},

			initialize: function(){
				this.listenTo(this.model, 'change:viewing', this.setCssHashState);
			...
			toggleTray: function(e){
				...
				var $tray = $this.parents('.drawer-list-item').find('.settings-outer');
				$tray.toggleClass('active', !$tray.hasClass('active'));

				// Center it
				// this.helpers.centerishInViewport($modal.find('.modal-content'))
			},
			...
			closeModal: function(e){
				...
				// $('.drawer-list-item').removeClass('modal-active'); // Bring back the `cursor: pointer` style
			},

			showModal: function(e){
				this.stopIt(e);
				var $this = $(e.currentTarget);
				$this.parents('.drawer-list-item').addClass('modal-active'); //  A hack to get the cursor property to go back to default
				var $modal = $this.parents('.drawer-list-item').find('.modal');
				$modal.toggleClass('active', !$modal.hasClass('active'));
			...
			setCssHashState: function(model){
				...
				var behavior = this.determineBehavior();
				routing.router.set[behavior]( this.model.get(app.instance.listUid), this.model.get('viewing') );
				return this;
			},
			...
			helpers: {
				centerishInViewport: function($el){
					...
					$el.css({
						top: v_height/2 - (el_height) + 'px',
						left: v_width/2 - (el_width/2) + 'px'
					})
				}
			}
		...
		ArticleDetailCard: Backbone.View.extend({
			...
			className: 'article-detail-card-wrapper',
			...
			close: function(){
				// collections.detail_items.instance.remove( this.model );
				var where_obj = {};
				where_obj[app.instance.listUid] = this.model.get(app.instance.detailUid);
				collections.drawer_items.instance.where(where_obj)[0].set('viewing', false);
			},
