			...
			articleDetail: {
				...
				bake: function(detailModel){
					...
					if (mode == 'single'){
						...
						if (!app.instance.$isotopeCntnr) app.instance.articleDetail.isotope.initCntnr();
						item_view = new views.ArticleDetailRow({model: detailModel});
						this.$content.find('#compare-grid .rows').append(item_view.render().el);
						item_view.update(this.articleDetail.isotope.addItem);
				...
				isotope: {
					initCntnr: function(){
						app.instance.$isotopeCntnr = app.instance.$content.find('.rows');
						app.instance.$isotopeCntnr.isotope({
							itemSelector: '.article-detail-row-wrapper',
							layoutMode: 'masonry'
						});
					},
					addItem: function($el){
						app.instance.$isotopeCntnr.isotope('appended', $el);
					}
				},
			},
			divisionSwitcher: {
