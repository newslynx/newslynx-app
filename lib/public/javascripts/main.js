(function(){
	...
	var routing = {
		init: {
			...
			"approval-river": function(){
				this.route('', function(){ routing.router.navigate('my-alerts/all'); });
		...
		helpers: {
			itemDetail: {
				...
				fetch: function(itemUid, cb){
					...
					var loaded_matches = _.filter(this.detailData, function(obj) { return obj.uid === itemUid });
