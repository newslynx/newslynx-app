	...
	window.app = {
		...
		Settings: Backbone.View.extend({
			...
			addItem: function(e){
				...
				// Set keys on add from the parent's `data-child-keys` attribute which is a serialized array.
				new_item.keys = [];
				if ($inputsContainer.attr('data-child-keys')) new_item.keys = JSON.parse($inputsContainer.attr('data-child-keys'))
				...
				if (valid){
					...
					this.model.save(settings, {
						...
						success: function(model, response, options){
							console.log('saved in model save');
						}
					});
				}
			},
			...
			getSettingsData: function(){
				...
				$inputContainers.each(function(i){
					...
					if ( $inputs.length && !$inputs.hasClass('multi') ){
