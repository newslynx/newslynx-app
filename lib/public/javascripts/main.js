	...
	mainData.alerts = [
		...
		},
		{
			...
			"settings": {
				...
				"language": "English",
				"region": "Any",
				"how_many": "Only the best results"
			}
		},
		{
			"uid": "a4",
	...
	mainData.alertCreators = [
		{
			...
			schema: {
				...
				result_type: {
					type: "select",
					options: ["Everything","News","Blogs","Video","Discussion","Books"]
				},
				language: {
					type: "select",
					options: ["English","Spanish","Esperanto"]
				},
				region: {
					type: "select",
					options: ["Any"]
				...
				how_many: {
					type: "select",
					options: ["Only the best results","All results"]
				}
			}
		},{
			service: "Twitter",
			...
			schema: {
				...
				search_query: {
					...
					help: {
						...
						hint: "site:propalpatine.org AND \"rebel alliance\""
					}
				}
			}
		}
	...
	var templates = {
		...
		helpers: {
			...
			bakeAlertUpdateForm: function(uid, service, settings){
				...
				$.extend(true, schema, _.filter(mainData.alertCreators, function(alertCreator){ return alertCreator.service == service })[0].schema);
				...
				text: function(service, prettyName, data){
					...
					markup = '<div class="form-row">';
						...
						markup += '<div class="input-text-container">';
							if (data.help && data.help.link) markup += '<div class="help-row"><a href="'+data.help.link+'" target="_blank">What do I search for?</a></div>'; 
