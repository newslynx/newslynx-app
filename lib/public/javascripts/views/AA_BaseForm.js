views.AA_BaseForm = Backbone.View.extend({
	...
	formJsonToMarkup: {
		...
		'article-search': function(fieldName, data, isDefaultEvent){
			...
			var input_markup = '<input type="text" name="'+fieldName+'" class="'+fieldName+'" placeholder="'+((data.help && data.help.hint) ? this.escapeQuotes(data.help.hint) : '') +'" data-is-default-event="'+isDefaultEvent+'" data-serialize-skip="true"/>';
		...
		datepicker: function(fieldName, data, isDefaultEvent){
			...
			input_markup = '<input type="text" name="'+fieldName+'" class="'+fieldName+'" placeholder="'+((data.help && data.help.hint) ? this.escapeQuotes(data.help.hint) : '') +'" autocomplete="off" data-is-default-event="'+isDefaultEvent+'"/>';
			
			return input_markup;
		},

		textOrNumber: function(fieldName, data, which, isDefaultEvent){
			...
			var value = this.escapeQuotes(data.selected),
					input_markup = '<input type="'+which+'" name="'+fieldName+':'+((which == 'text') ? 'string' : which)+'" class="'+fieldName+'" value="'+this.escapeQuotes(value)+'" placeholder="'+((data.help && data.help.hint) ? this.escapeQuotes(data.help.hint) : '') +'" '+((which == 'number') ? 'min="0"' : '') +' data-is-default-event="'+isDefaultEvent+'"/>';
			return input_markup;

		},

		text: function(fieldName, data, isDefaultEvent){
			return this.formJsonToMarkup.textOrNumber.call(this, fieldName, data, 'text', isDefaultEvent);
		},
		...
		number: function(fieldName, data, isDefaultEvent){
			return this.formJsonToMarkup.textOrNumber.call(this, fieldName, data, 'number', isDefaultEvent);
		},

		paragraph: function(fieldName, data, isDefaultEvent){
			var value = this.escapeQuotes(data.selected) || '',
					input_markup = '<textarea type="text" name="'+fieldName+'" class="'+fieldName+'" placeholder="'+((data.help && data.help.hint) ? this.escapeQuotes(data.help.hint) : '') +'" data-is-default-event="'+isDefaultEvent+'">'+value+'</textarea>';
