views.AA_BaseForm = Backbone.View.extend({
	...
	initArticleTitleSearcher: function(){
		...
		// TODO, hack for now, on recipe creators, `this.schema` is the recipe schema, not the default event
		// This should probably be standardized so that schema is always called event or something
		...
		// // And then reverse it
		// Not necessary because `event_schema` passed by reference
		// if (!this.schema.assignees) {
		// 	this.default_event = event_schema;
		// } else {
		// 	this.schema = event_schema;
		// }
	...
	addArticleAssignee: function(d){
		...
		// Currently, selecting an article won't remove it from the typeahead options, so as a fix, we'll make sure this list of ids is always unique
	...
	initPikaday: function(){
		...
		if (event_schema.timestamp && event_schema.timestamp.selected){
			time_picker.setDate(new Date(event_schema.timestamp.selected*1000));
		}
