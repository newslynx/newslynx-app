views.AA_BaseForm = Backbone.View.extend({
	...
	formJsonToMarkup: {
		...
		datepicker: function(fieldName, data, isDefaultEvent){
			...
			var input_markup = '<input type="text" data-type="datepicker" class="'+class_name+'" placeholder="'+((data.help && data.help.placeholder) ? this.escapeQuotes(data.help.placeholder) : '') +'" autocomplete="off" data-is-default-event="'+isDefaultEvent+'"/>';
			// Make an input sibling that will be what we read the data from
			input_markup += '<input type="hidden" data-type="datepicker-value" class="'+class_name+'" data-is-default-event="'+isDefaultEvent+'"/>';
		...
		time_picker = new Pikaday({
			...
			onSelect: function(){
				var moment_timezone_date = this.getMoment(),
						...
						pretty_date_string = moment_timezone_date.format('MMM D, YYYY, h:mm a'); // June 23, 2014, 9:13 am
