views.AA_BaseForm = Backbone.View.extend({
	...
	formJsonToMarkup: {
		...
		textOrNumber: function(fieldName, data, which){
			var value = this.escapeQuotes(data.selected) || '',
					input_markup = '<input type="'+which+'" name="'+fieldName+'" class="'+fieldName+'" value="'+value+'" placeholder="'+((data.help && data.help.hint) ? this.escapeQuotes(data.help.hint) : '') +'" '+((which == 'number') ? 'min="0"' : '') +'/>';
		...
		checkbox: function(fieldName, data){
			var input_markup  = '<span class="placeholder">You haven\'t yet made any '+fieldName.replace(/_/g,' ')+' yet. Create them on the <a href="/settings" target="_blank" class="out-link">Settings</a> page.</span>',
	...
	drag: d3.behavior.drag()
					.on("drag", function(d,i) {
						...
						// As a fix, only drag the window if no input fields are selected
