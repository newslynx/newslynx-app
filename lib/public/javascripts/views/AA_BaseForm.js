views.AA_BaseForm = Backbone.View.extend({
	...
	formJsonToMarkup: {
		...
		timestamp: function(fieldName, data){
			...
			// input_markup += '<input type="text" data-value="'+
		...
		paragraph: function(fieldName, data){
			var value = this.escapeQuotes(data.selected) || '',
					input_markup = '<textarea type="text" name="'+fieldName+'" class="'+fieldName+'" value="'+value+'" placeholder="'+((data.help && data.help.hint) ? this.escapeQuotes(data.help.hint) : '') +'"/>';
