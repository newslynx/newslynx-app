views.AA_BaseForm = Backbone.View.extend({
	...
	assignmentTemplateFactory: _.template('<div class="article-assignee""><span class="remove-assignee labelled" aria-label="<%= title %>" >&times;</span><input type="hidden" name="content_item_ids[]:number" value="<%= id %>"/></div>'),
	...
	addArticleAssignee: function(d){
		...
		if (d){
			...
			if (!_.contains(form_data.content_item_ids, id)){
