views.AA_BaseForm = Backbone.View.extend({
	...
	assignmentTemplateFactory: _.template('<div class="article-assignee""><span class="remove-assignee labelled" aria-label="<%= title %>" >&times;</span><input type="hidden" name="content_items[]:number" value="<%= id %>"/></div>'),
	...
	initArticleTitleSearcher: function(){
		...
		// if (_.isArray(this.event_schema.content_items.selected)){
		// 	this.event_schema.content_items.selected.forEach(function(contentItem){
	...
	addArticleAssignee: function(d){
		...
		if (d){
			...
			if (!_.contains(_.pluck(form_data.content_items, 'id'), id)){
	...
	combineFormSchemaWithVals: function(schema_obj, settings_obj){
		...
		_.each(settings_obj, function(setting, fieldName){
			...
			if (fieldName != 'tag_ids' && fieldName != 'content_items'){
