views.AA_BaseForm = Backbone.View.extend({
	...
	formJsonToMarkup: {
		...
		content_items: function(assignees, setEventMode){
			var markup = '';
		...
		set_event_content_items: function(assignees){
			return this.formJsonToMarkup.content_items.call(this, assignees, true);
		},
		...
		datepicker: function(fieldName, data, isDefaultEvent){
			...
			var input_markup = '<input type="text" data-type="datepicker" name="'+fieldName+'" class="'+class_name+'" placeholder="'+((data.help && data.help.placeholder) ? this.escapeQuotes(data.help.placeholder) : '') +'" autocomplete="off" data-is-default-event="'+isDefaultEvent+'"/>';
	...
	initArticleTitleSearcher: function(setEventMode){
		...
		// Depending on what kind of form we're making, the content items will be stashed on different objects
		// This is a by-product of how we separate out form_data from event_schema
		if (setEventMode){
			content_items = this.event_schema.set_event_content_items;
		} else {
			content_items = this.form_data.content_items;
		}
	...
	initPikaday: function(){
		...
		time_picker = new Pikaday({
			...
			onSelect: function(){
				...
				that.form_data.timestamp = timestamp/1000;
			},
			onClear: function(){
