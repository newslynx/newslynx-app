views.AA_BaseForm = Backbone.View.extend({
	...
	formJsonToMarkup: {
		...
		textOrHidden: function(fieldName, data, which, isDefaultEvent){
			...
			// Text fields that are empty will be passed back as null
			var input_markup = '<input type="'+which+'" name="'+fieldName+':null" class="'+class_name+'" value="'+this.escapeQuotes(value)+'" placeholder="'+((data.help && data.help.placeholder) ? this.escapeQuotes(data.help.placeholder) : '') +'" data-is-default-event="'+isDefaultEvent+'"/>';
		...
		number: function(fieldName, data, isDefaultEvent){
			...
			// Don't pass text fields back as null becuase sometimes we want `0` to be our value
			var input_markup = '<input type="number" name="'+fieldName+':auto" class="'+class_name+'" value="'+this.escapeQuotes(value)+'" placeholder="'+((data.help && data.help.placeholder) ? this.escapeQuotes(data.help.placeholder) : '') +'" min="0" data-is-default-event="'+isDefaultEvent+'"/>';
			
			return input_markup;
		},

		text: function(fieldName, data, isDefaultEvent){
			return this.formJsonToMarkup.textOrHidden.call(this, fieldName, data, 'text', isDefaultEvent);
		},

		hidden: function(fieldName, data, isDefaultEvent){
			return this.formJsonToMarkup.textOrHidden.call(this, fieldName, data, 'hidden', isDefaultEvent);
		},
