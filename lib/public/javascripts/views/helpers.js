views.helpers = {
	...
	centerishInViewport: function($el){
		...
		$el.css({
			top:  _.min(.25, (((v_height/2 - (el_height/2))/v_height*100) - 10)) + '%',
		...
		var inputObj = d3.nest()
		    ...
		    .rollup(function(list){
		        ...
		        if (list.length == 1 && list[0].name != 'impact_tags' && list[0].name != 'assignees') { 
		        	...
		        	return list.map(function(f) { return f.value }); 
		        }
		    })
		    .map(inputArrays);
...
*/
	...
	remodelEventJson: function(alert_id, formInputsArray){
		...
		if (input_obj.assignees && input_obj.assignees.length){
			input_obj.assignees = input_obj.assignees.map(function(assignee){ return +assignee; })
		} else {
			input_obj.assignees = [];
		}
