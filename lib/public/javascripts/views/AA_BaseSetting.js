views.AA_BaseSetting = Backbone.View.extend({
	...
	events: {
		...
		'change .js-input-item[type="text"]': 'inputHasChanged',
	...
	getVal: function(dataLocation){
		...
		// var new_obj = {};
		// if (_.isEmpty(saved_data) && value_keys.length > 1){
		// 	new_obj[first_val] = {};
		// 	val = new_obj;
		// }
	...
	revertToPreviousSettingVal: function(e){
		e.preventDefault();
		console.log(e)
		// For some reason, a click is triggered on this button when we hit the return key
		// That event, however, has `0` for pageX or pageY values. 
		// So this is a hack but it will prevent the UI from displaying the old data
		// TODO, find a better way to not have cancel feel that it is "clicked" on "enter"
		if (e.pageX && e.pageY){
			var $form = this.$form,
	...
	saveModel: function(e){
		...
		if (attrs_to_save.options){
			existing_attrs = _.pick(this.model.get('options'), _.identity);
