views.AA_BaseSetting = Backbone.View.extend({
	...
	getRelevantSavedFormData: function(){
		...
		var current_form_data = this.model.get('input_val') || this.getCurrentFormData();

		var form_keys = Object.keys(current_form_data);
	...
	getVal: function(dataLocation){
		var location = dataLocation || this.valueKey || this.options.valueKey,
				value_keys = dataLocation.replace(/\]/g, '').split('['),
	...
	saveModel: function(e){
		...
		if (attrs_to_save.options){
			...
			attrs_to_save.options = _.extend(attrs_to_save.options, existing_attrs);
		}
