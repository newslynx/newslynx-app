views.AA_BaseSetting = Backbone.View.extend({
	...
	// Override this function in your Setting type and it will be run on init
	...
	initialize: function(){
		...
		// this.listenTo(this.model, 'save', this.modelSaved);

		// Our custom view as defined on a per view basis, such as how to fill default values and such
		this.viewInitialize();
	...
	initColorPicker: function($el){
		...
		this.$el.find('.color-picker').each(function(){
			...
			var group = $colorPicker.parents('.js-inputs-container').attr('data-model'); // `'subject_tag'` or `'impact_tag'`
