views.AA_BaseSetting = Backbone.View.extend({
	...
	getVal: function(dataLocation){
		var location = dataLocation || this.options.valueKey,
				value_keys = location.replace(/\]/g, '').split('['),
				relevant_saved_data = this.getRelevantSavedFormData(),
				val = relevant_saved_data[value_keys[0]]; // Use the underscore `get` method for the first value

		// If we have specified any other values through a syntax of `key[nested_key][other_nested_key]`, then loop through them and repalce the val
		_.chain(value_keys).rest().each(function(value_key){
			val = val[value_key];
		}, this).value();
		return val;
	},
