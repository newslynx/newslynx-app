views.TagSectionNav = views.Tag.extend({
	...
	setOpts: function(tagModel, isActive){
		...
		// The API doesn't distinguish so these will go under the same query param
		if (group == 'subject_tags' || group == 'impact_tags'){
			group = 'tag_ids';
		}
	...
	getImportantModelInfo: function(){
		...
		// So do some testing if that came out to empty
		if (_.isEmpty(info)){
