views.ArticleComparisonGrid = Backbone.View.extend({
	...
	initialize: function(){
		...
		// this.metrics = this.hydrateMetrics(metrics);
	...
	render: function(){
		var select_dimensions = _.extend({
				...
				sort: this.collection.metadata('sort')
			}, 
	...
	setMetricHeaders: function(){
		...
		this.metrics.forEach(function(quantMetric){
			...
			value = '<span class="comparison-figure-operation">'+display_operation_abbreve+': </span>' + value;

			$header_el.html(value).attr('aria-label', display_operation + ' of ' + group + ' articles.');
		}, this);
	}
})
