views.ArticleSummaryRow = Backbone.View.extend({
	...
	render: function() {
		var $el = this.$el,
				...
				data_for_template = _.extend(
					{
						view: this, // We're gonna wanna call calcSize from within our template so pass the context so it can maintain a reference to `this`
						selects: this.collection.getSelectDimensions(),
						calcSize: this.calcSize
					}, 
					model_json, 
					helpers.templates.articles),
				article_detail_markup = templates.articleSummaryRowFactory( data_for_template ),
				subject_tag_len,
				impact_tag_len;
		...
		console.log(this.collection.getSelectDimensions())
		...
		this.d3_el = d3.select( this.el ).select('.article-detail-row-container').selectAll('.cell');
	...
	calcComparisonMarkerParams: function(){
		...
		this.comparison_marker_max 				= collections.article_comparisons.instance.metadata('max'); // per_975
		return this;
	},
		...
		var markers = this.bullet_markers
				...
				.styleTween('left', function(d) { 
					...
					var starting_px = parseFloat(d3.select(this).style('left')),
							...
							ending_percent = that.helpers.calcSize.call(that, d.metric, that.comparison_marker_operation, that.comparison_marker_max),
	...
	// helpers: {
		calcSize: function(metric, val, field, group){
			...
			console.log(this.comparison_marker_max)
			console.log(this.comparison_marker_group)

			var this_metrics_info = _.findWhere(models.comparison_metrics.get(group), {metric: metric}),
					max,
					scale,
					val_percent;
			...
			// if (this_metrics_info){
				// max = this_metrics_info[field] || 1000;
				max =  100000;
