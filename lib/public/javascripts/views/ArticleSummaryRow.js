views.ArticleSummaryRow = Backbone.View.extend({
		...
		var markers = this.bullet_markers
				...
				.styleTween('left', function(d) { 
					...
					var starting_px = parseFloat(d3.select(this).style('left')),
							...
							ending_percent = that.helpers.calcSize(d.metric, that.comparison_marker_dimension, that.comparison_marker_max),
	...
	helpers: {
		calcSize: function(metric, val, field){
			...
			var this_metrics_info = _.findWhere(pageData.articleComparisons, {metric: metric}),
