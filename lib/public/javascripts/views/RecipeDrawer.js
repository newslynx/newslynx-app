views.RecipeDrawer = Backbone.View.extend({
	...
	filter: function(){
		var that = this,
				...
				alerts_in_memory = collections.loaded_alerts.instance.where({recipe_id: recipe_id}); // Gets the first model that matches the recipe, which, because this collection is sorted will be the newest one


		// Do some cleanup
		// Get rid of the load more button
		app.instance.clearLoadMoreButton.call(app.instance);
		...
		var page_size = collections.active_alerts.instance.metadata('page_size');

		var fetch_options = {
					data: { 
						recipe_id: recipe_id,
						include_raw: true
					}, // To be populated down below with `after` if applicable.
					processData: true,
					success: function(collection, response, options){
						// `active_alerts` now contains the fetched stuff. Also add what we had in memory
						collections.active_alerts.instance.add(alerts_in_memory);
		...
		collections.active_alerts.instance.metadata('recipe_id', recipe_id);


		// See if we need to offset by any existing timestamp		
		var oldest_timestamp_in_memory;
		...
		if (fetching){
			// Empty this collection first
			collections.active_alerts.instance.set([]);
			// Fetch new alerts, callbacks specified in `fetch_options`.
			// TEMPORARY, this page size will be set on the server
			fetch_options.data.count = page_size;
