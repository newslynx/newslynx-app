		...
		checkbox: function(fieldName, fieldNamePretty, data){
			...
			if (!_.contains(banished_keys, fieldNamePretty)){
				markup = '<div class="form-row">';
					...
					markup += '</div>';
					markup += '<div class="form-row-input-container">';
					data.options.forEach(function(checkboxItem){
						var checkboxId = _.uniqueId(checkboxItem + '-nl_id-'); // `form.serializeArray()` will turn this into a data value so later on we'll to remove the number from this value since it will needed to become a generic property name on the key. 
						markup += '<div class="form-checkbox-group">'
							markup += '<input id="'+checkboxId+'" name="'+checkboxId+'" type="checkbox" />';
