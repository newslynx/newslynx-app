helpers.templates = {
	...
	toUserTimezone: function(utcDate){
		...
		var utc_date = new Date(utcDate),
				user_timezone_date = new Date(new Date(utcDate).setHours(utc_date.getHours() + pageData.timezone ));
	...
	prettyTimestamp: function(utcDate){
		return new Date(utcDate*1000).toLocaleString().replace(/\:[0-9][0-9] /, ' ').toLowerCase();
	},
