...
views.AA_BaseForm = Backbone.View.extend({
	...
	formJsonToMarkup: {
		...
		'radio': function(fieldName, data, isDefaultEvent, selectedVal, type){
			var input_markup = '',
		...
		_.each(csv_schemas, function(schema, csvName){
			...
			_.each(schema, function(keyNames, groupType){
				keyNames.forEach(function(keyName, index){
					row[keyName] = app.helpers.exportData[groupType].call(delimiters, schema_data, keyName, index);
				});
			});
...
app.Articles = Backbone.View.extend({
	...
	comparison: {
		...
		loadRows: function(stagedArticleComparisonModels, saveHash){
			console.log(collections.article_comparisons.instance.length)
			if (collections.article_comparisons.instance.length){
...
app.helpers = {
	...
	exportData: {
		...
		objectLists: function(data, objs){
