	...
	getSettings: function(setDefaultEvent){
		...
		var form_options = $form_selector.serializeJSON({
			...
			customTypes: {
				...
				// Use the values in all of the data-attibutes to make an object
				// Useful for making json lists
				'data-attributes-object': function(val){
					return $(val).data()
				}
			}
		});
...
views.MetricsPicker = views.AA_BaseForm.extend({
  ...
  template: _.template($('#metric-picker-item').html()),
...
var init = {
	go: function(){
		...
		collections.init[section].call(collections, function(err){
			if (!err) {
