...
helpers.templates = {
    ...
    toUserTimezone: function(utcDate) {
        utcDate = utcDate * 1e3;
        var utc_date = new Date(utcDate), user_timezone_date = new Date(new Date(utcDate).setHours(utc_date.getHours() + pageData.timezone));
...
views.AA_BaseArticleViz = Backbone.View.extend({
    ...
    render: function() {
        ...
        _columns.append("div").classed("bar-container", true).append("div").classed("bar", true).style("width", function(d) {
            return (d.pageviews / that.total * 100).toFixed(2) + "%";
        }).append("div").classed("label", true).html(function(d) {
...
views.ArticleComparisonGrid = Backbone.View.extend({
    ...
    hydrateMetrics: function(quantMetrics) {
        var metric_maxes = pageData.articleComparisons.filter(function(metricMax) {
...
views.ArticleDetail = Backbone.View.extend({
    ...
    initialize: function() {
        ...
        var that = this;
        this.spottedTail = spottedTail().timezoneOffset(pageData.timezone).y(function(d) {
            return +d.count;
    ...
    bakeArticleVizs: function() {
        var $articleVizsContainer = this.$el.find('section.detail-section[data-group="reading"]');
        ...
        var device_facet_markup = device_facet_view.render().el;
        $articleVizsContainer.append(device_facet_markup);
        ...
        var internal_external_facet_markup = internal_external_facet_view.render().el;
        $articleVizsContainer.append(internal_external_facet_markup);
        ...
        $articleVizsContainer.append(domain_facet_markup);
...
views.ArticleSummaryRow = Backbone.View.extend({
    ...
    helpers: {
        calcSize: function(metric, val, field) {
            ...
            var this_metrics_info = _.findWhere(pageData.articleComparisons, {
                metric: metric
...
views.RecipeDrawer = Backbone.View.extend({
    ...
    bakeRecipeEditor: function() {
        var event_editor_view = new views.RecipeEditor({
            el: this.el,
...
views.RecipeEditor = views.AA_BaseForm.extend({
    ...
    initialize: function(opts) {
        ...
        var recipe_schema = $.extend(true, {}, _.findWhere(pageData.recipeSchemas, {
            source: source
        }));
...
views.RecipeSchemaForm = Backbone.View.extend({
    ...
    printSubmitMsg: function(error, msg) {
        ...
        this.$submitMsg.addClass(class_name).html(msg).delay(3e3).fadeOut(500).delay(750).queue(function(next) {
