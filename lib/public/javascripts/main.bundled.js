...
helpers.common = {
    ...
    Model: Backbone.Model.extend({
        initialize: function() {
            ...
            this.on("change", function() {
                var mode = that.get("mode"), current_ids = that.current_ids;
    ...
    Collection: Backbone.Collection.extend({
        ...
        metadata: helpers.modelsAndCollections.metadata,
        set: function() {
            Backbone.Collection.prototype.set.apply(this, arguments), this.updateHash();
        },
        remove: function() {
            Backbone.Collection.prototype.remove.apply(this, arguments), this.updateHash();
        },
        updateHash: function() {
    ...
    getAssociatedAlertsForRecipe: {
        go: function(recipeModel) {
            ...
            min_timestamp = collections.all_alerts.instance.metadata("timestamp"), this.getAssociatedAlertsForRecipe.fetch.call(this, recipe_id, pending, min_timestamp)), 
            this;
        },
    ...
    initialize: function() {
        ...
        this.$divisionSwitcher = $(".division-switcher"), this.$drawerPointersCntnr = $("#drawer-pointers-container"), 
        this.isotopeCntnr = ".rows", this.isotopeChild = ".article-detail-row-wrapper", 
        this.listId = "id", this.detailId = "article_id", this.listenTo(models.section_mode, "change:mode", this.sectionMode.update), 
    ...
    bake: function() {
        return this.$drawerPointersCntnr.append(templates.drawerPointers), collections.subject_tags.instance.each(function(tag) {
    ...
    sectionMode: {
        update: function(model, mode) {
            return mode = mode || model.get("mode"), this.sectionMode[mode].call(this, model, mode), 
            this;
        },
        compare: function() {
            var article_grid = new views.ArticleComparisonGrid();
            this.$content.html(article_grid.render().el), this.$listContainer = $("#compare-grid .rows"), 
            app.helpers.isotope.initCntnr.call(this);
            var compare_models = collections.article_comparisons.instance.models;
            return compare_models.length || (compare_models = collections.article_summaries.instance.where({
                selected: !0
    ...
    drawer: {
        ...
        remove: function(summaryModel) {
            return summaryModel.set("in_drawer", !1), this;
        }
    },
    addToComparison: function(e) {
        var sort_by, sort_ascending, action, $btn = $(e.currentTarget), action = $btn.attr("data-action"), selected_models = collections.article_summaries.instance.where({
            selected: !0
        });
        "replace" == action ? action = "set" : "add" == action && (action = "add"), collections.article_comparisons.instance[action](selected_models), 
        sort_by = collections.article_comparisons.instance.metadata("sort_by"), sort_ascending = collections.article_comparisons.instance.metadata("sort_ascending"), 
        app.helpers.isotope.relayout(sort_by, sort_ascending), this.saveHash();
    },
    comparison: {
        add: function(summaryModel) {
            var item_view, item_el;
            return item_view = new views.ArticleSummaryRow({
                model: summaryModel
            }), item_el = item_view.render().el, this.$listContainer.append(item_el), app.helpers.isotope.addItem.call(app.instance, item_el), 
            this;
        },
        remove: function(comparisonModel) {
            return comparisonModel.set("destroy", "delete"), this;
        }
    },
    saveHash: function() {
    ...
    getSettingsData: function() {
        ...
        return $inputContainers.each(function() {
            ...
            }) : (input_val = $inputs.find(".input-item").val(), input_val && (settings[key] = input_val.trim()));
        }), settings;
    },
    ...
    isotope: {
        initCntnr: function() {
            this.$isotopeCntnr = this.$listContainer, this.$isotopeCntnr.isotope({
        ...
        relayout: function(sortBy, sortAscending) {
            sortBy = sortBy || "timestamp", _.isUndefined(sortAscending) && (sortAscending = !1), 
    ...
    initialize: function() {
        this.listenTo(this.model, "change:selected", this.setActiveCssState), this.listenTo(this.model, "change:in_drawer", this.destroy);
    },
    render: function() {
    ...
    render: function() {
        ...
        this.data = this.model.toJSON(), _.each(this.data.quant_metrics, function(bullet) {
    ...
    transformData: function(modelData) {
        ...
        return modelData.qual_metrics = d3.nest().key(function(d) {
            ...
            var tags = $.extend(!0, [], list.map(function(d) {
                return pageData.orgInfo[d.metric].filter(function(f) {
                    return f.id == d.id;
                })[0];
            }));
    ...
    close: function() {
        collections.article_comparisons.instance.remove(this.model), app.instance.saveHash();
    },
    destroy: function(model, destroyMode) {
        "delete" == destroyMode && (app.instance.$isotopeCntnr ? app.instance.$isotopeCntnr.isotope("remove", this.$el).isotope("layout") : this.remove(), 
        model.set({
            destroy: !1
        }, {
            silent: !0
    ...
    helpers: {
        calcSize: function(d, value) {
            var val = d[value], max = pageData.orgInfo.metric_maxes.filter(function(f) {
    ...
    Router: Backbone.Router.extend({
        ...
        loadRecipe: function(mode, ids) {
            ...
            "my-recipes" == mode && app.instance.model.set("view-all", !1), this.setModeOnly(mode), 
            ids.exiting && this.addRemove.call(app.instance, mode, ids.exiting, !1), ids.entering && this.addRemove.call(app.instance, mode, ids.entering, !0);
        },
        loadAllInSection: function(mode) {
            this.setModeOnly(mode), this.enableAll.call(app.instance);
        },
        ...
        compareArticles: function(ids) {
            ...
            var compare_models, is_compare_mode = "compare" == models.section_mode.get("mode");
            is_compare_mode ? (compare_models = ids.split("&").map(function(id) {
        ...
        addRemove: function(mode, ids, show) {
            ids && collections.recipes.instance.setBoolByIds("viewing", this.listId, ids, show);
        },
        enableAll: function() {
    ...
    helpers: {
        diffIds: function(mode, newIds) {
            ...
            obj);
            {
                var previous_ids = this.history[this.history.length - 1].ids.split("&"), newIds = newIds.split("&");
                previous_ids.concat().sort(helpers.common.sortNumber), newIds.concat().sort(helpers.common.sortNumber), 
                this.history[this.history.length - 1].mode;
            }
            return "my-recipes" != mode || previous_ids || (previous_ids = app.helpers.drawer.getAllIds.call(app.instance)), 
            "my-recipes" == mode && (obj.entering = newIds.join("&"), obj.exiting = previous_ids.join("&")), 
            obj;
        },
    ...
    articles: function() {
        ...
        this.drawerPointers = $("#drawer-pointers-templ").html(), this.articleGridContainerMarkup = $("#article-grid-container-templ").html(), 
    ...
    articles: function() {
        this.section_mode = new models.section_mode.Model(), this.section_mode.compare = {};
    },
    "approval-river": function() {
        this.section_mode = new models.section_mode.Model(), this.section_mode.set("mode", "my-recipes"), 
    ...
    articles: function() {
        ...
        console.log(this.article_summaries.instance.pluck("timestamp")), this.article_comparisons.instance = new this.article_comparisons.Collection([]);
    },
    "approval-river": function() {
        ...
        this.active_alerts.instance = new this.active_alerts.Collection([]), this.all_alerts.instance = new this.all_alerts.Collection(pageData.alerts.results), 
    ...
    common: function() {
        this.starting_route && this.route("", function() {
    ...
    articles: function() {
        this.route(":mode", "setModeOnly"), this.route("compare/:ids", "compareArticles"), 
        this.starting_route = "compare";
    },
    "approval-river": function() {
        this.route(":mode", "loadAllInSection"), this.route(":mode/:id", "loadRecipe"), 
