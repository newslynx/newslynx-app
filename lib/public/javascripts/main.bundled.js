...
app.Articles = Backbone.View.extend({
    ...
    initialize: function() {
        ...
        collections.po.article_summaries.on("queryChange", this.drawer.setActiveArticleSummaries);
        views.po.article_summaries.on("pageChange", this.drawer.addActiveArticleSummaries);
    ...
    sectionMode: {
        ...
        compare: function(model, mode) {
            ...
            app.helpers.isotope.initCntnr.call(this);
            this.$drawer.find('.drawer-item-group[data-which="comparison-additions"] input,.drawer-item-group[data-which="comparison-additions"] button').prop("disabled", false).parent().removeClass("disabled");
        ...
        detail: function() {
            this.$content.html("");
            this.$drawer.find('.drawer-item-group[data-which="comparison-additions"] input,.drawer-item-group[data-which="comparison-additions"] button').prop("disabled", true).parent().addClass("disabled");
    ...
    moreSummaryArticles: function() {
        views.po.article_summaries.page(1);
    },
...
views.ArticleDetailAccountSubjectTag = Backbone.View.extend({
    ...
    styleLayout: function() {
        var bg_color = this.model.get("color"), text_color = this.whiteOrBack(bg_color), bg_color_darker = this.colorLuminance(bg_color, -.1);
...
views.ArticleSummaryDrawerImpactTag = Backbone.View.extend({
    ...
    styleLayout: function() {
        var bg_color = this.getColor(), text_color = this.whiteOrBack(bg_color), bg_color_darker = this.getBorderColor();
        this.$el.css({
            "background-color": bg_color,
            color: text_color
        });
...
views.ArticleSummaryRow = Backbone.View.extend({
    ...
    redrawMarker: function() {
        ...
        var that = this;
        var markers = this.bullet_markers.style("left", function(d) {
    ...
    enter: function(cb) {
        ...
        row.append("div").classed("cell", true).classed("title", true).classed("wide", true).classed("tooltipped", true).attr("data-tooltip-align", "left").attr("data-id", function(d) {
            return d.id;
...
collections.init = {
    articles: function() {
        ...
        this.po.article_summaries.addSorts([ timestamp_sort_desc ]);
...
app.init = {
    articles: function() {
        ...
        collections.article_summaries.instance.set(summaries_page_one);
