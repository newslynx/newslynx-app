	...
	addArticle: function(e){
		...
		this.validate(this.form_info.schema, form_data, function(err, msg){
			if (!err){
				...
				// this.collection.sync('create', form_data, {
				// 	wait: true,
...
views.TagEventFilter = views.Tag.extend({
	...
	setOpts: function(tagModel, isActive){
		...
		filter_model.metadata(group, has_tags);
...
views.TagSectionNav = views.Tag.extend({
	...
	setOpts: function(tagModel, isActive){
		...
		var tag_list = models.content_item_filters.get(combined_group) || [];
		...
		if (has_tags){
			models.content_item_filters.set(combined_group, tag_list);
		} else {
			models.content_item_filters.unset(combined_group);
		}
...
app.Articles = Backbone.View.extend({
	...
	bakeArticleAdder: function(){
		...
		// Create an instance of an event creator view
		var add_article_view = new views.AddArticle({defaults: defaults, el: this.$addArticle[0], collection: collections.article_summaries.instance});
