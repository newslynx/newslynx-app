...
helpers.common = {
    ...
    itemDetail: {
        go: function(listItemModel) {
            var destination, action, is_new = listItemModel.get("viewing"), id = listItemModel.get(this.listId), pending = 9, min_timestamp = collections.all_detail_items.instance.meta("timestamp");
            return is_new ? (destination = app.helpers.itemDetail.add, action = "add") : (destination = app.helpers.itemDetail.remove, 
            action = "remove"), app.helpers.itemDetail.fetch.call(this, id, action, destination, pending, min_timestamp), 
            this;
        },
        fetch: function(itemId, action, cb, pending, minTimestamp) {
            var mode = models.section_mode.get("mode"), loaded_matches = collections.all_detail_items.instance.filterAlerts(app.instance.detailId, itemId);
            ...
            app.instance.$isotopeCntnr.isotope({
                ...
                minTimestamp && (options = {
                    ...
                    error: function() {
                        console.log("Error fetching " + itemId), cb([]);
                    }
                });
            }
        },
        ...
        remove: function(itemData) {
            ...
            itemData.forEach(function(itemDatum) {
                var detailId = itemDatum[that.detailId], where_obj = {};
                where_obj[that.detailId] = detailId, collections.detail_items.instance.remove(collections.detail_items.instance.where(where_obj)[0]);
            });
        },
        removeAll: function() {
    ...
    saveModal: function(e) {
        ...
        new_event_model.save(alert_data, {
            ...
            success: function(model, response) {
                console.log("saved recipe", response), this.removeItem("save"), views.helpers.toggleModal(e);
            }
        });
    },
    remodelFormJson: function() {
        var serializedArray = this.$form.serializeArray(), model_json = views.helpers.remodelEventJson(serializedArray);
        return model_json;
    },
    ...
    remodelEventJson: function(formInputsArray) {
        var input_obj = views.helpers.formSerialToObject(formInputsArray);
        return input_obj.impact_tags.length && (input_obj.impact_tags = input_obj.impact_tags.map(function(impactTag) {
    ...
    helpers: {
        diffIds: function(mode, newIds) {
            ...
            return console.log("previous id", previous_ids), "my-recipes" != mode || previous_ids || (previous_ids = app.helpers.drawer.getAllIds.call(app.instance)), 
    ...
    "approval-river": function() {
        ...
        console.log(pageData.riverItems.min_timestamp), this.all_detail_items.instance.meta("timestamp", pageData.riverItems.min_timestamp);
    },
    settings: function() {}
}, app.init = {
    articles: function() {
