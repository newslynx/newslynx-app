...
views.AA_BaseForm = Backbone.View.extend({
    ...
    formJsonToMarkup: {
        "article-search": function(fieldName, data) {
            fieldName = "assignees-selector";
            var input_markup, selecteds = data.selected;
            data.selected = null;
        ...
        paragraph: function(fieldName, data) {
            var value = this.escapeQuotes(data.selected) || "", input_markup = '<textarea type="text" name="' + fieldName + '" class="' + fieldName + '" value="' + value + '" placeholder="' + (data.help && data.help.hint ? this.escapeQuotes(data.help.hint) : "") + '"></textarea>';
            return input_markup;
        },
        ...
        checkbox: function(fieldName, data) {
            ...
            _.each(data.options, function(checkboxItemObj) {
                ...
                var checkboxId = _.uniqueId("NewsLynx|checkbox|" + fieldName + "|" + checkboxItemObj.name + "|");
    ...
    initPikaday: function() {
        var time_picker, el = this.$el.find('input[name="timestamp"]')[0];
