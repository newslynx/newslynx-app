...
helpers.common = {
    ...
    formJsonToMarkup: {
        ...
        checkbox: function(fieldName, fieldNamePretty, data) {
            ...
            markup += "</div>", markup += '<div class="form-row-input-container">', data.options.forEach(function(checkboxItem) {
                var checkboxId = _.uniqueId(checkboxItem + "-nl_id-");
                markup += '<div class="form-checkbox-group">', markup += '<input id="' + checkboxId + '" name="' + checkboxId + '" type="checkbox" />', 
    ...
    toggleDefaults: function(e) {
        var $btn = $(e.currentTarget), open = "Enabled" == $btn.html();
        console.log(open), open ? $btn.html("Disabled").attr("data-status", "closed") : $btn.html("Enabled").attr("data-status", "open"), 
