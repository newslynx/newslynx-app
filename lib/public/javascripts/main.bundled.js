...
helpers.common = {
    ...
    bakeRecipeUpdateForm: function(uid, source, settings, recipeName, schemaOrDefaultEvent) {
    ...
    Collection: Backbone.Collection.extend({
        ...
        filterAlerts: function(uidKey, searchVal) {
            var where_obj = {};
            return where_obj[uidKey] = searchVal, this.where(where_obj);
        }
    })
    ...
    initialize: function() {
        ...
        }, this.listUid = "uid", this.detailUid = "recipe_id", this.listenTo(collections.drawer_items.instance, "change:viewing", app.helpers.itemDetail.go), 
    ...
    divisionSwitcher: {
        updateHash: function(entering_mode) {
            var exiting_hash = window.location.hash, exiting_mode = routing.helpers.getMode(exiting_hash), exiting_uids = routing.helpers.getArticleUids(exiting_hash), previous_uids = models.section_mode.get("previous-uids") || "", entering_hash = entering_mode;
    ...
    drawer: {
        ...
        add: function(mode, uids) {
            app.helpers.drawer.setModeOnly(mode), uids && collections.drawer_items.instance.setBoolByIds("viewing", this.listUid, uids, !0);
        },
    ...
    itemDetail: {
        go: function(listItemModel) {
            var destination, action, is_new = listItemModel.get("viewing"), uid = listItemModel.get(this.listUid), pending = 9, min_timestamp = collections.detail_items.instance.meta("timestamp");
            return is_new ? (destination = app.helpers.itemDetail.add, action = "add") : (destination = app.helpers.itemDetail.remove, 
            action = "remove"), app.helpers.itemDetail.fetch.call(this, uid, action, destination, pending, min_timestamp), 
            this;
        },
        fetch: function(itemUid, action, cb, pending) {
            var mode = models.section_mode.get("mode"), loaded_matches = collections.all_detail_items.instance.filterAlerts(app.instance.detailUid, itemUid);
            if (loaded_matches.length && "my-recipes" != mode || "my-recipes" == mode && loaded_matches.length == pending) cb.call(this, loaded_matches), 
            app.instance.$isotopeCntnr.isotope({
                ...
                min_timestamp && (options = {
                    ...
                    error: function() {
                        console.log("Error fetching " + itemUid), cb([]);
                    }
                });
            }
        },
    ...
    setActiveCssState: function(model) {
        ...
        return uids ? (this.$el.find(".drawer-list-outer").toggleClass("active", model.get("viewing")), 
    ...
    destroy: function() {
        ...
        "delete" == destroy_mode ? (console.log("deeelete", app.instance.$isotopeCntnr), 
        app.instance.$isotopeCntnr ? app.instance.$isotopeCntnr.isotope("remove", this.$el) : this.remove()) : "save" == destroy_mode && (app.instance.$isotopeCntnr ? app.instance.$isotopeCntnr.isotope("remove", this.$el).isotope("layout") : this.remove());
    }
    ...
    Router: Backbone.Router.extend({
        ...
        readPage: function(mode, uids) {
            mode = mode.replace(/\//g, ""), console.log(uids), uids = routing.helpers.diffUids.call(this, mode, uids), 
            "my-recipes" == mode && app.instance.model.set("view-all", !1), uids.exiting && app.helpers.drawer.remove.call(app.instance, mode, uids.exiting), 
    ...
    helpers: {
        diffUids: function(mode, newUids) {
            ...
            if (!this.history.length || !this.history[this.history.length - 1].uids) return this.history[this.history.length - 2] && "my-recipes" != mode ? {} : (obj.entering = newUids, 
            obj);
            ...
            return console.log("previous uid", previous_uids), "my-recipes" != mode || previous_uids || (previous_uids = app.helpers.drawer.getAllUids.call(app.instance)), 
    ...
    "approval-river": function() {
        ...
        this.detail_items.instance = new this.detail_items.Collection([]), this.detail_items.instance.sort = "datetime", 
