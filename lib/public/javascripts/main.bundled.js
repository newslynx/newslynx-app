...
helpers.templates = {
    ...
    prettyTimestamp: function(utcDate) {
        return new Date(utcDate * 1e3).toLocaleString().replace(/\:[0-9][0-9] /, " ").toLowerCase();
    },
    ...
    displaySearchParams: function(source, settings) {
        if (source == "google-alert") return settings.search_query; else if (source == "twitter-search") return settings.q; else if (source == "twitter-list") return settings.slug + "," + settings.filter; else if (source == "twitter-user") return settings.screen_name + "," + settings.filter; else if (source == "reddit-search") return settings.search_query; else if (source == "facebook-page") return settings.page_id + "," + settings.filter; else console.error("You need to add a search_query key `templates.js` to display this type of recipe.");
    },
...
views.Alert = Backbone.View.extend({
    ...
    createEvent: function(e) {
        ...
        var required_keys = [ "timestamp", "assignees", "title", "what_happened", "significance", "impact_tags" ];
...
views.EventEditor = views.AA_BaseForm.extend({
    ...
    createEvent: function(e) {
        ...
        var required_keys = [ "timestamp", "title", "what_happened", "significance", "impact_tags" ];
        this.validate(required_keys, function(error, description) {
            var that = this;
            if (!error) {
