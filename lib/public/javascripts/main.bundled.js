...
views.ArticleSummaryRow = Backbone.View.extend({
    ...
    transformData: function(modelData) {
        ...
        modelData.qual_metrics = d3.nest().key(function(d) {
            return d.metric;
        }).rollup(function(list) {
            var tags = $.extend(true, [], list.map(function(d) {
        ...
        modelData.qual_metrics["impact_tags"] = d3.nest().key(function(d) {
            return d.category;
        }).rollup(function(list) {
            return {
                count: list.length,
    ...
    update: function(cb) {
        ...
        row.append("div").classed("cell", true).classed("title", true).classed("wide", true).attr("data-article_id", function(d) {
            return d.article_id;
        }).html(function(d) {
            return d.title;
        });
        row.append("div").classed("cell", true).classed("date", true).classed("single", true).attr("data-date", function(d) {
            return d.timestamp;
        }).html(function(d) {
            return helpers.templates.conciseDate(d.timestamp);
        });
        var bullet_container = this._el.data(this.data.quant_metrics).enter().append("div").classed("cell", true).classed("multi", true).classed("gfx", true).append("div").classed("bullet-container", true);
        var that = this;
...
views.Tag = Backbone.View.extend({
    ...
    styleLayout: function() {
        ...
        this.$el.toggleClass("active", is_active).find(".tag-container").css("background-color", color);
        return this;
    },
    toggle: function() {
