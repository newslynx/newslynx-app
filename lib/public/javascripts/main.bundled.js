...
views.ArticleDetailVizDeviceFacet = views.AA_BaseArticleViz.extend({
    initialize: function(options) {
        ...
        device_facets.forEach(function(deviceFacet) {
            var existing_val = _.findWhere(device_facets_full, {
    ...
    render: function() {
        ...
        }).append("div").classed("label", true).style("color", "red").html(function(d) {
...
views.ArticleDetailVizInternalExternal = views.AA_BaseArticleViz.extend({
    ...
    render: function() {
        ...
        _columns.append("div").classed("bar-container", true).append("div").classed("bar", true).style("width", function(d) {
            ...
            var percent = Math.round(d.pageviews / that.total * 100), count = d.pageviews ? "(" + d.pageviews + ")" : "";
            return "<strong>" + helpers.templates.toTitleCase(d.facet) + "</strong> &mdash; " + percent + "% " + count;
        });
        return this;
    }
