...
helpers.common = {
    ...
    getAssociatedAlertsForRecipe: {
        go: function(recipeModel) {
            var recipe_id, pending, min_timestamp, destination, action, is_new = recipeModel.get("viewing");
            return is_new && (recipe_id = recipeModel.get(this.listId), pending = recipeModel.get("pending"), 
            min_timestamp = collections.all_alerts.instance.meta("timestamp"), destination = this.getAssociatedAlertsForRecipe.add, 
            action = "add", this.getAssociatedAlertsForRecipe.fetch.call(this, recipe_id, action, destination, pending, min_timestamp)), 
            this;
        },
        fetch: function(recipe_id, action, cb, pending, minTimestamp) {
            var page_limit = 5, options = {}, mode = models.section_mode.get("mode"), loaded_matches = collections.all_alerts.instance.filterAlerts(app.instance.detailId, recipe_id);
            ...
            app.helpers.isotope.relayout()) : (minTimestamp && (options = {
                data: {
                    before: minTimestamp,
                    recipe_id: recipe_id
                },
        ...
        add: function(itemData) {
            collections.active_alerts.instance.set(itemData), app.instance.$isotopeCntnr.isotope("layout");
        }
    },
    divisionSwitcher: {
    ...
    river: {
        ...
        loadAllAlerts: function(loadAllModel) {
            ...
            load_all && (console.log("load all"), collections.active_alerts.instance.set(collections.all_alerts.instance.models), 
    ...
    "approval-river": function() {
        ...
        this.all_alerts.instance.meta("timestamp", pageData.alerts.min_timestamp);
    },
    settings: function() {}
}, app.init = {
    articles: function() {
