...
helpers.templates = {
    ...
    bakeRecipeUpdateForm: function(uid, source, settings) {
        ...
        })[0].schema), schema_with_selects = this.combineFormSchemaWithVals(schema, settings), 
        markup = this.bakeForm(source, schema_with_selects);
    },
    combineFormSchemaWithVals: function(schema_obj, settings_obj) {
        return _.each(settings_obj, function(setting, fieldName) {
            schema_obj[fieldName].selected = setting;
        }), schema_obj;
    },
    formJsonToMarkup: {
        text: function(source, prettyName, data) {
            var markup, name_id = _.uniqueId(source), value = this.escapeQuotes(data.selected) || "";
            return markup = '<div class="form-row">', markup += '<label for="' + name_id + '"> ' + prettyName + "</label> ", 
            markup += '<div class="input-text-container">', data.help && data.help.link && (markup += '<div class="help-row"><a href="' + data.help.link + '" target="_blank">How do I search?</a></div>'), 
        ...
        select: function(source, prettyName, data) {
            var markup, name_id = _.uniqueId(source);
            return markup = '<div class="form-row">', markup += "<label>" + prettyName + "</label> ", 
            markup += '<select id="' + name_id + '">', _.each(data.options, function(option) {
                var selected = "";
                data.selected == option && (selected = "selected"), markup += '<option value="' + option + '" ' + selected + ">" + option + "</option>";
            }), markup += "</select>", markup += "</div>";
        },
    ...
    prettyName: function(name) {
        return name = name.replace(/_/g, " "), name.charAt(0).toUpperCase() + name.slice(1);
    },
...
app.ApprovalRiver = Backbone.View.extend({
    ...
    bake: function() {
        return collections.drawer_items.instance.each(function(recipe) {
            ...
            model: models.section_mode.instance,
            el: this.$divisionSwitcher
        }), app.helpers.isotope.initCntnr.call(this), this;
    },
    resetFull: function() {
        return routing.router.navigate(models.section_mode.instance.get("mode") + "/all", {
            trigger: !0
        }), this;
    },
    scrollTo: function(e) {
    ...
    divisionSwitcher: {
        updateHash: function(entering_mode) {
            var exiting_hash = window.location.hash, exiting_mode = routing.helpers.getMode(exiting_hash), exiting_uids = routing.helpers.getArticleUids(exiting_hash), previous_uids = models.section_mode.instance.get("previous-uids") || "all", entering_hash = entering_mode;
            "my-recipes" == exiting_mode && exiting_uids ? models.section_mode.instance.set("previous-uids", exiting_uids) : "create-new" == exiting_mode && previous_uids && (entering_hash += "/" + previous_uids), 
        ...
        bake: function(detailModel) {
            {
                ...
                models.section_mode.instance.get("mode"), detailModel.get("uid");
            }
...
app.Articles = Backbone.View.extend({
    ...
    articleDetail: {
        ...
        bake: function(detailModel) {
            var item_view, item_el, mode = models.section_mode.instance.get("mode");
    ...
    divisionSwitcher: {
        ...
        updateHash: function(entering_mode, new_uid) {
            ...
            var hash_arr = current_uids = [], new_uids = new_uid || models.section_mode.instance.get("previous-uids") || "";
            ...
            models.section_mode.instance.set("previous-uids", current_uids), this;
        }
    },
    drawer: {
        filter: function() {
...
app.helpers = {
    drawer: {
        ...
        determineBehavior: function() {
            var behavior = "radio";
            return "compare" == models.section_mode.instance.get("mode") && (behavior = "checkbox"), 
            behavior;
        },
        getAllUids: function() {
            var uids = [];
    ...
    itemDetail: {
        ...
        fetch: function(itemUid, action, cb) {
            var mode = models.section_mode.instance.get("mode"), loaded_matches = _.filter(this[mode].detailData, function(obj) {
...
views.DrawerListItem = Backbone.View.extend({
    ...
    saveModal: function(e) {
        console.log("Saved"), this.toggleModal(e);
    },
    toggleEnabled: function(e) {
        e.stopPropagation(), this.model.set("enabled", !this.model.get("enabled"));
    },
...
views.RecipeForm = Backbone.View.extend({
    ...
    render: function() {
        ...
        return this.$el.html(river_item_markup).attr("data-mode", "create-new"), this;
    },
    save: function() {}
});
    ...
    Router: Backbone.Router.extend({
        ...
        set: {
            radio: function(uid, trigger) {
                _.isUndefined(trigger) && (trigger = !0), routing.router.navigate(models.section_mode.instance.get("mode") + "/" + uid, {
                    trigger: trigger
                });
            },
...
var templates = templates || {}, models = models || {}, collections = collections || {}, app = app || {}, views = views || {}, routing = routing || {};
