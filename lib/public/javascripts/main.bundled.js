...
collections.article_detailed_events = {
	"Collection": Backbone.Collection.extend({
		...
		metadata: helpers.modelsAndCollections.metadata,
		url: 'api/_VERSION/events?facets=tags,categories,levels&status=approved&per_page=10&creates=events&sous_chefs=!twitter-search-content-item-links-to-event', 
		parse: function(response){
	...
	"Collection": Backbone.Collection.extend({
		...
		setSelects: function(cb){
			...
			self.fetchSelects(function(err, dimensionsSelects, selectsDefaults){
				var groups = ['dimensions', 'sort_order', 'timeseries']
				var settings
				if (err){
					console.log('Error getting selects', err)
				} else {
					settings = groups.map(function(group){
						var setting = _.findWhere(dimensionsSelects, {name: group})
						if (!setting) {
					...
					self.metadata('timeseries-selects', settings[2]);
...
views.MetricsPicker = views.AA_BaseForm.extend({
  ...
  render: function( ){
    ...
    current_selects.forEach(function(selectDimension){
      ...
      var $tmp = $('<div></div>')
      $(this.template()(selectDimension)).appendTo($tmp)
    ...
    not_select_dimensions.forEach(function(nonSelectDimension){
      ...
      markup += this.template(nonSelectDimension)
    }, this)
  ...
  updateSelectDimensions: function(e){
    ...
    // TKTK
    console.log(select_dimensions)
    // var dSetting = new models.user_setting.Model(select_dimensions)
