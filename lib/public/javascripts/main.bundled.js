...
collections.article_detailed_subject_tags = {
    ...
    Collection: Backbone.Collection.extend({
        ...
        url: function() {
            ...
            return "api/articles/" + article_id + "/subject";
        },
        set: function() {
            Backbone.Collection.prototype.remove.call(this, this.models);
            Backbone.Collection.prototype.set.apply(this, arguments);
...
views.ArticleDetail = Backbone.View.extend({
    ...
    initialize: function() {
        ...
        this.spottedTail = spottedTail().x(function(d) {
            var utc_date = new Date(d.timestamp * 1e3), user_timezone_date = new Date(new Date(utcDate).setHours(utc_date.getHours() + parseFloat(pageData.orgInfo.timezone)));
            return user_timezone_date;
        }).y(function(d) {
            return +d.count;
...
views.ArticleSummaryRow = Backbone.View.extend({
    ...
    render: function() {
        ...
        this.data.quant_metrics.splice(6, 1);
        this.data.quant_metrics.splice(5, 1);
        this.data.quant_metrics.splice(3, 1);
    ...
    update: function(cb) {
        ...
        bullet_container.append("div").classed("bullet", true).style("width", function(d) {
            return that.helpers.calcSize(d, "value");
        });
