...
helpers.templates = {
    ...
    combineFormSchemaWithVals: function(schema_obj, settings_obj, recipeName) {
        ...
        _.each(settings_obj, function(setting, fieldName) {
            ...
            if (fieldName != "impact_tags" && fieldName != "assignee") {
    ...
    formJsonToMarkup: {
        text: function(fieldName, fieldNamePretty, data) {
            ...
            markup += '<input type="text" name="' + fieldName + '" class="' + fieldName + '" value="' + value + '" placeholder="' + (data.help && data.help.hint ? this.escapeQuotes(data.help.hint) : "") + '"/>';
            markup += "</div>";
            markup += "</div>";
            return markup;
        },
    ...
    doesThisArticleHaveThisTag: function(tagName) {
        ...
        console.log(this_articles_tag_names, tagName);
        if (has_this_tag) {
            return_val = "checked";
        }
        return return_val;
...
app.Articles = Backbone.View.extend({
    ...
    detail: {
        ...
        loadPage: function(model) {
            var subject_tags_full, impact_tag_categories, impact_tag_levels;
...
views.Alert = Backbone.View.extend({
    ...
    initArticleTitleSearcher: function() {
        console.log(app.bloodhound.ttAdapter());
        this.$el.find("." + this.typeaheadInputId).typeahead({
            highlight: true
        }, {
            name: "articles",
            displayKey: "title",
...
views.ArticleSummaryDrawer = Backbone.View.extend({
    ...
    setActiveCssState: function() {
        var selected = this.model.get("selected"), id = this.model.get("id");
        if (selected) {
        ...
        _.findWhere(views.po.article_summaries.getCurrentItems(), {
            id: id
        }).selected = selected;
        return this;
    },
...
views.helpers = {
    ...
    formSerialToObject: function(formInputsArray) {
        var inputArrays = formInputsArray.map(function(inputObj) {
            var name_parts;
            if (inputObj.name.indexOf("impact_tags|") != -1 || inputObj.name.indexOf("assignee|") != -1) {
...
collections.init = {
    articles: function() {
        ...
        this.po.article_summaries.addFilters([ subject_tags_filter, impact_tags_filter, impact_tag_category_filter, impact_tag_level_filter, text_search_filter ]);
        ...
        for (var i = 0; i < load_n_articles; i++) {
            summary_models[i].set("selected", true);
        }
