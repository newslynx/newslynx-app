...
app.Articles = Backbone.View.extend({
    ...
    bakeArticleAdder: function() {
        var defaults = {};
        var add_article_view = new views.AddArticle({
            defaults: defaults,
            el: this.$addArticle[0]
        });
...
views.AA_BaseForm = Backbone.View.extend({
    ...
    formJsonToMarkup: {
        ...
        checkbox: function(fieldName, data) {
            var input_markup = '<span class="placeholder">You haven\'t yet made any ' + fieldName.replace(/_/g, " ") + ' yet. Create them on the <a href="/settings" target="_blank" class="out-link">Settings</a> page.</span>', namespacer = "NewsLynx";
...
views.AddArticle = views.AA_BaseForm.extend({
    ...
    addArticle: function(e) {
        ...
        var required_keys = [ "timestamp", "url", "title", "authors" ];
        this.validate(required_keys, function(error, description) {
            if (settings_obj.authors) {
                settings_obj.authors = settings_obj.authors.split(",").map(function(author) {
                    return author.trim();
                });
            }
            var that = this;
...
views.ArticleSummaryRow = Backbone.View.extend({
    ...
    transformData: function(modelData) {
        ...
        var missing_metrics, existing_metrics;
...
app.init = {
    articles: function() {
        ...
        console.log(summaries_page_one);
