    ...
    render: function() {
        ...
        tags_container.selectAll(".tag").data(function(d) {
            ...
            var tag_view = new views.ArticleSummaryDrawerImpactTag({
                model: tag_model
            }), tag_markup = tag_view.render().$el.html(), tag_color = tag_view.getColor(), tag_label = tag_view.getLabel();
...
views.ArticleSummaryDrawerImpactTag = Backbone.View.extend({
    ...
    styleLayout: function() {
        var bg_color = this.getColor(), text_color = this.whiteOrBack(bg_color);
        this.$el.css({
            "background-color": bg_color,
            color: text_color
        });
