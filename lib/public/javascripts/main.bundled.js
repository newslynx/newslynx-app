	...
	formJsonToMarkup: {
		...
		'checkbox-single': function(fieldName, data, isDefaultEvent, selectedVal, type){
			...
			data.input_options.forEach(function(checkboxItemObj, idx){
				...
				input_markup += '<input id="'+checkboxId+'" name="'+fieldName+'[]:boolean" value="false" type="checkbox" ' + checked + ' data-is-default-event="'+isDefaultEvent+'"/>';
...
views.ArticleDetail = Backbone.View.extend({
	...
	events: {
		...
		'click #download-data': 'setDownloadButton'
	},
	...
	setDownloadButton: function(e){
		...
		// Comparisons csv
		csvs[timestamp+'_article_'+this.model.id+'_comparisons'] = flattenObj(models.comparison_metrics.toJSON()).map(addArticleMetrics(model_json.metrics))
		...
		function flattenObj(comparisonObj) {
			...
			_.each(comparisonObj, function(subGroup, groupName){
				...
				if (_.isArray(subGroup)){
					...
					_.each(subGroup, function(metricsList, subGroupName){
						...
						_.extend(metric_row, metricsList)
						csv.push(metric_row);
					})
					// groupMetrics.forEach(function(groupMetric){
					// 	var group_metric = {
					// 		group: groupName
					// 	};
					// 	_.extend(group_metric, groupMetric);
					// 	csv.push(group_metric);
					// })
		...
		function addArticleMetrics(modelJson){
			return function(objToAugment){
				var augmented_obj = {}
				var metric = modelJson[objToAugment.metric];
				augmented_obj['article_'+objToAugment.metric]
		...
		models.exports.fetch({
			...
			.then(function(){
				...
				// console.log(csvs)
				// console.log(models.exports.toJSON())
			// 	_.extend(csvs, models.export_data.toJSON())
...
views.ArticleSummaryRow = Backbone.View.extend({
	...
	calcSize: function(metric, val, max, group){
		...
		if (this_metrics_info){
			...
			console.log('ERROR: Missing comparison values for group', group, 'and metric', metric, 'in comparison object', comparison_object);
			val_percent = '0%';
		}
		return val_percent;
	}
...
app.helpers = {
	...
	exportData: {
		...
		// Assumes it has `id` and `name` keys
		objectLists: function(data, key){
			return data[key].map(function(obj){
				return [obj.id, obj.name].join(this.secondary)
