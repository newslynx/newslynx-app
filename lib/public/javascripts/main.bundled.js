...
views.AA_BaseForm = Backbone.View.extend({
    ...
    initArticleTitleSearcher: function() {
        ...
        var event_schema;
        if (!this.schema.assignees) {
            event_schema = this.default_event;
        } else {
            event_schema = this.schema;
        }
        if (_.isArray(event_schema.assignees.selected)) {
            event_schema.assignees.selected.forEach(function(assignee) {
    ...
    initPikaday: function() {
        ...
        if (event_schema.timestamp && event_schema.timestamp.selected) {
            time_picker.setDate(new Date(event_schema.timestamp.selected * 1e3));
        }
...
views.RecipeCreator = views.AA_BaseForm.extend({
    ...
    render: function() {
        var markup = "", recipe_schema = this.schema, default_event_markup;
    ...
    renderDefaultEvent: function() {
        var markup = "", default_event_schema = this.default_event;
    ...
    createRecipe: function(e) {
        e.preventDefault();
        var settings_obj = this.schema;
        settings_obj.default_event = this.default_event;
