...
views.ArticleDetail = Backbone.View.extend({
    ...
    initialize: function() {
        ...
        views.po.article_detailed_events.on("update", this.eventsGallery.setActiveEvents);
        ...
        this.spottedTail = spottedTail().x(function(d) {
            ...
            return +d.count;
        }).legend(this.legend).eventSchema(pageData.impactCategoriesInfo).events(this.eventsDataForST).interpolate("step-after").onBrush(this.filterEventsByDateRange);
    },
    render: function() {
...
views.ArticleDetailEvent = Backbone.View.extend({
    ...
    initialize: function() {
        ...
        this.listenTo(this.model, "change:in_selection", this.update);
    },
    render: function() {
        ...
        }).enter().append("li").classed("tag", true).each(function(d) {
            var tag_model = new models.subject_tag.Model(d);
            var tag_view = new views.ArticleSummaryDrawerImpactTag({
                model: tag_model
            }), tag_markup = tag_view.render().$el.html(), tag_color = tag_view.getColor();
