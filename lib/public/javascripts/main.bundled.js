...
helpers.common = {
    ...
    Collection: Backbone.Collection.extend({
        ...
        parse: function(response) {
            return this.metadata("min_timetamp", response.min_timetamp), response.results;
        }
    })
    ...
    initialize: function() {
        ...
        this.isotopeChild = ".article-detail-wrapper", this.listId = "id", this.detailId = "recipe_id", 
        ...
        this.bake(), this.$content.on("scroll", this.lazyLoadAlerts);
    },
    ...
    bake: function() {
        return collections.recipes.instance.each(function(recipe) {
            ...
            this.$recipes.append(recipe_view.render().el), this.pending_total += recipe.get("pending");
    ...
    getAssociatedAlertsForRecipe: {
        ...
        fetch: function(recipe_id, pending, minTimestamp) {
            var page_limit = 5, options = {}, that = this, mode = models.section_mode.get("mode"), loaded_matches = collections.all_alerts.instance.filterAlerts(app.instance.detailId, recipe_id);
            loaded_matches.length && "my-recipes" != mode || "my-recipes" == mode && (loaded_matches.length >= page_limit || loaded_matches.length == pending) ? (cb.call(this, loaded_matches), 
            app.helpers.isotope.relayout()) : (options = {
                processData: !0,
                data: {
                    before: minTimestamp,
                    recipe_id: recipe_id
                ...
                success: function(collection, response, options) {
                    console.log(options), that.getAssociatedAlertsForRecipe.cleanupFetch(collection, response, options);
                },
                error: function() {
        ...
        cleanUp: function() {},
        cleanupFetch: function(collection, results, options) {
            collections.all_alerts.instance.add(results), collections.active_alerts.instance.metadata("active_recipe_id", options.data.recipe_id), 
    ...
    river: {
        bake: function(detailModel) {
            ...
            return detailModel.set("destroy", !1), item_view = new views.Alert({
                model: detailModel
    ...
    lazyLoadAlerts: function() {
        var $content = $(this), content_scrollHeight = $content[0].scrollHeight, content_scrollTop = $content.scrollTop(), content_outerHeight = $content.outerHeight(), at_bottom = content_scrollHeight - content_scrollTop == content_outerHeight, active_recipe_id = (collections.active_alerts.instance.metadata("min_timestamp"), 
        collections.active_alerts.instance.metadata("active_recipe_id")), options = {};
        at_bottom && (options = {
            remove: !1,
            processData: !0,
            data: {
