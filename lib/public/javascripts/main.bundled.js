	...
	/** Metric options: per97_5, per75, median, per25, per2_5, per5, per95, mean **/
		...
		if (this_metrics_comp_info){
			...
			if (typeof val == 'string') {
				val = this_metrics_comp_info[val];
				if (!val){
...
views.ArticleDetail = Backbone.View.extend({
	...
	setDownloadButton: function(){
		...
		// Article info
		csv_schemas[timestamp+'_article_'+this.model.id] = {
			singles: ['id','created', 'description', 'domain', 'site_name', 'title', 'type', 'updated','url'],
			objects: ['authors','subject_tags_full', 'impact_tags_full'],
		...
		_.each(csv_schemas, function(schema, csvName){
			...
			_.each(schema, function(keyNames, groupType){
				keyNames.forEach(function(keyName, index){
					row[keyName] = app.helpers.exportData[groupType].call(delimiters, model_json, keyName, index);
				});
			});
		...
		// Events csv
		console.log(this.article_detailed_events_collection.toJSON())
		// csvs[timestamp+'_article_'+this.model.id+'_events'] = this.article_detailed_events_collection.toJSON().map(addExportedDate)
		...
		var device_facets = [
			{
				...
				value: 50000 // TEMPORARY until this gets filled in by API
			},{
...
views.ArticleDetailVizDomainFacets = views.AA_BaseArticleViz.extend({
	...
	initialize: function(options){
		...
		domain_facets_from_article.forEach(function(facetInfo){
...
app.helpers = {
	...
	exportData: {
		...
		objects: function(data, objs){
			return data[objs].map(function(obj){
		...
		metricsLists: function(data, metricName, index){
			var metric_name = metricName.split('|')[0]; // This is a nested metric that we expanded. So we stored its parent name and then the facet name delimited by '|'
			return data.metrics[metric_name][index].value
		}
