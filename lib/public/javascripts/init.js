...
collections.init = {
	...
	"approval-river": function(){
		...
		this.loaded_alerts.instance = new this.loaded_alerts.Collection(pageData.alerts.results);
		this.loaded_alerts.main_river_instance = new this.loaded_alerts.Collection(pageData.alerts.results);
	...
	settings: function(){
		...
		this.user_settings.instance = new this.user_settings.Collection(pageData.userSettingsList);
...
app.init = {
	...
	"approval-river": function(){
		var page_size = 25; // This can be any number below the API max, which is currently 500. Check the docs if that has recently changed.
		...
		collections.loaded_alerts.main_river_instance.metadata('total_pending', pageData.alerts.total);

		collections.active_alerts.instance.metadata('total_pending', pageData.alerts.total);
		collections.active_alerts.instance.metadata('page_size', page_size);
	...
	settings: function(){
		this.defaults.rss_feed_recipe_name = 'Ingest Articles from an RSS Feed';
		this.defaults.staff_twitter_list_to_promotion_recipe_name = 'Staff Twitter List to Promotion Event';
		this.defaults.twitter_user_to_promotion_recipe_name = 'Staff Twitter User to Promotion Event';
		this.defaults.facebook_page_to_promotion_recipe_name = 'Staff Facebook Page to Promotion Event';
