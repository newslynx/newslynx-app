...
models.init = {
	articles: function(){
		...
		this.content_item_filters = new models.content_item_filters.Model({sort: pageData.articleSummariesInfo.sort});
...
collections.init = {
	articles: function(){
		...
		// The more consistent way is to make impact_tag_attributes two separate collection classes but I don't like making SO many of those it seems ridiculous.
		...
		this.dimensions.instance.metadata('sort', pageData.dimensionsInfo.sort);
		...
		// TODO, set this on initalize to cut down on all the stuff here
...
app.init = {
	articles: function(){
		...
		// TODO, similarly define these settings on init
		collections.article_comparisons.instance.metadata('sort_by', 'ga_pageviews');
...
var init = {
	common: {
		...
		// 					return -1;
		// 				} else if (a.metrics[key] < b.metrics[key]){
		// 					return 1;
		// 				} else {
		// 					return 0;
		// 				}
