function spottedTail() {
	...
	function chart(selection_) {
		...
		selection.each(function(data, idx) {
			...
			var continuous_g = eventItems_continuous.append('g')
				...
				.attr('transform', function(d) { return 'translate('+(margin.left + xScale(d.datetime[0]))+',0)' }) // Offset this one so the top border can show

			continuous_g.append('rect')
				.classed('ST-event-rect', true)
				.attr('x', '0')
				.attr('width', function(d){ return xScale(d.datetime[1]) - xScale(d.datetime[0]) })
			...
			var discrete_g = eventItems_discrete.append('g')
				...
				// .classed('ST-tooltipped', true)
				.attr('transform', function(d,i) { return 'translate('+(margin.left + xScale(d.datetime) )+',14)' })
			...
			brush_categories_container.selectAll('.ST-event-continuous-brush')
				...
				.enter()
					.append('rect')
						...
						.attr('x', function(d){ return xScale(d.datetime[0]) })
			...
			function brushed(d) {
				...
				eventTimelineCntnr.selectAll('.ST-event-circle-g').attr('transform', function(d,i) { return 'translate('+(margin.left + xScale(d.datetime))+',14)'; });
				var rect_g = eventTimelineCntnr.selectAll('.ST-event-rect-g').attr('transform', function(d) { return 'translate('+(xScale(d.datetime[0]) + margin.left)+',1)' })
				rect_g.select('rect')
						.attr('width', function(d) { 
							return xScale(d.datetime[1]) - xScale(d.datetime[0]) 
						});

				rect_g.select('text')
						.attr('transform', function(d){
							var width = xScale(d.datetime[1]) - xScale(d.datetime[0]) 
							return 'translate('+width/2+','+(events_row_height*1.5)+')';
						});
			...
			function populateHoverWindow(hoverInfo){
				if (hoverInfo.metrics){
					...
					event_item_container.append('div')
						...
						.html(function(d){
							var timestamp;
							if (!Array.isArray(d.datetime)){
								timestamp = d.datetime.format('M/D, h:mma')
							} else {
								timestamp = d.datetime[0].format('M/D, h:mma') + ' to ' + d.datetime[1].format('M/D, h:mma')
							}
			...
			function setHoverInfoHighlightEvents(mouseCoords){
				...
				if (d0 && d1){
					var d = data_val_at_mouse_x - d0.datetime > d1.datetime - data_val_at_mouse_x ? d1 : d0,
				...
				categtory_selection.selectAll('.ST-event-rect')
					.each(function(d){
						var d3_this = d3.select(this),
								begin_x = xScale(d.datetime[0]),
								end_x   = xScale(d.datetime[1]),
								within_range = mouse_x > begin_x && mouse_x < end_x;

						d3_this.classed('ST-highlighted', false);
						if (within_range){
							d.begin_timestamp = d.datetime[0];
			...
			function transformPromotionsIntoSpots(promos){
				...
				promosMaxDate = d3.max(promos_copy, function(d) { 
					if (Array.isArray(d.datetime)){
						return d3.max(d.datetime) || new Date(0);
					} else {
						return d.datetime; 
					}
				});
				...
				promosMinDate = d3.min(promos_copy, function(d) { 
					if (Array.isArray(d.datetime)){
						return d3.min(d.datetime) || new Date();
					} else {
						return d.datetime; 
					}
				});
				...
				promos_copy = promos_copy.map(function(promo){
					var type = 'discrete';
					if (Array.isArray(promo.datetime)) {
						type = 'continuous';
					}
					promo.type = type;
			...
			function transformEventsIntoSpots(evts){
				...
				eventsMaxDate = d3.max(events_copy, function(d) { return d.datetime; }) || new Date(0);
				eventsMinDate = d3.min(events_copy, function(d) { return d.datetime; }) || new Date();
	...
	function parseDates(arr){
		var converted = arr.map(function(d) {
			...
			if (Array.isArray(d.datetime)) { 
				d.datetime = d.datetime.map(setTimezone);
			} else { 
				d.datetime = setTimezone(d.datetime);
			}
	...
	function setTimezone(utcTimestamp, index, list, timezoneRegion){
		...
		var utc_moment = moment(utcTimestamp),
				...
				console.log(utcTimestamp, user_timezone_moment.format())
