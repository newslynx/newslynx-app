function spottedTail() {
	...
	function chart(selection_) {
		...
		selection.each(function(data, idx) {
			...
			var continuous_g = eventItems_continuous.append('g')
				...
				.attr('transform', function(d) { return 'translate('+(margin.left + xScale(d.created[0]))+',0)' }) // Offset this one so the top border can show

			continuous_g.append('rect')
				.classed('ST-event-rect', true)
				.attr('x', '0')
				.attr('width', function(d){ return xScale(d.created[1]) - xScale(d.created[0]) })
			...
			brush_categories_container.selectAll('.ST-event-continuous-brush')
				...
				.enter()
					.append('rect')
						...
						.attr('x', function(d){ return xScale(d.created[0]) })
			...
			function brushed(d) {
				...
				rect_g.select('rect')
						.attr('width', function(d) { 
							return xScale(d.created[1]) - xScale(d.created[0]) 
						});

				rect_g.select('text')
						.attr('transform', function(d){
							var width = xScale(d.created[1]) - xScale(d.created[0]) 
							return 'translate('+width/2+','+(events_row_height*1.5)+')';
						});
					...
					event_item_container.append('div')
						...
						.html(function(d){
							...
							if (!Array.isArray(d.created)){
								created = d.created.format('M/D, h:mma')
							} else {
								created = d.created[0].format('M/D, h:mma') + ' to ' + d.created[1].format('M/D, h:mma')
							}
				...
				categtory_selection.selectAll('.ST-event-rect')
					.each(function(d){
						var d3_this = d3.select(this),
								begin_x = xScale(d.created[0]),
								end_x   = xScale(d.created[1]),
	...
	function parseDates(arr){
		var converted = arr.map(function(d) {
			...
			if (Array.isArray(d[key])) { 
				d[key] = d[key].map(setTimezone);
			} else { 
				d[key] = setTimezone(d[key]);
			}
	...
	function setTimezone(utcDatestamp, index, list, timezoneRegion){
