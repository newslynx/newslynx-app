function spottedTail() {
	...
	function chart(selection_) {
		...
		selection.each(function(data, idx) {
			...
			var discrete_g = eventItems_discrete.append('g')
				...
				// .classed('ST-tooltipped', true)
				.attr('transform', function(d,i) { return 'translate('+(margin.left + xScale(d.created) )+',14)' })
			...
			brush_categories_container.selectAll('.ST-events-discrete-brush')
				...
				.enter()
					.append('circle')
						...
						.attr('cx', function(d){ return xScale(d.created) })
			...
			function brushed(d) {
				...
				eventTimelineCntnr.selectAll('.ST-event-circle-g').attr('transform', function(d,i) { return 'translate('+(margin.left + xScale(d.created))+',14)'; });
				
				var rect_g = eventTimelineCntnr.selectAll('.ST-event-rect-g').attr('transform', function(d) { return 'translate('+(xScale(d.created[0]) + margin.left)+',1)' })
				rect_g.select('rect')
					...
					event_item_container.append('div')
						...
						.html(function(d){
							...
							if (!Array.isArray(d.created)){
								timestamp = d.created.format('M/D, h:mma')
							} else {
								timestamp = d.created[0].format('M/D, h:mma') + ' to ' + d.created[1].format('M/D, h:mma')
							}
	...
	function parseDates(arr){
		var converted = arr.map(function(d) {
			...
			var t = d.datetime || d.created;

			if (Array.isArray(t)) { 
				d[time_string] = t.map(setTimezone);
			} else { 
				d[time_string] = setTimezone(t);
			}
