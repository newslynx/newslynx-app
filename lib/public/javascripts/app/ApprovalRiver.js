app.ApprovalRiver = Backbone.View.extend({
	...
	content: {
		...
		setActiveAlertsPerRecipe: function(recipeId){
			...
			// Add the `'0'` to protect against the count being zero.
			var recipe_alerts_pending_count = loaded_alerts_collection.metadata('total') || '0' || collections.recipes.instance.findWhere({id: recipeId}).get('event_counts').pending;
			// Convert that zero to a number
			recipe_alerts_pending_count = +recipe_alerts_pending_count;
