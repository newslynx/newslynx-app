app.ApprovalRiver = Backbone.View.extend({
	...
	river: {
		bake: function(detailModel){
			...
			detailModel.urlRoot = 'api/alerts'; // This should be inheriting from models.river_item.Model or even the collection url but it's not.
		...
		loadAllAlerts: function(loadAllModel){
			...
			if (load_all){
				...
				collections.detail_items.instance.each(function(m){
					console.log(m.url)
