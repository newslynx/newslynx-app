app.ApprovalRiver = Backbone.View.extend({
	...
	sectionMode: {
		...
		create: function(){
			...
			collections.sous_chefs.instance.each(function(recipeSchema){
	...
	content: {
		...
		setActiveAlertsPerRecipe: function(recipeId){
			...
			// console.log(collections.active_alerts.instance.metadata('recipe_id'))
			// console.log(collections.active_alerts.instance)
			// console.log('###')
			...
			var recipe_alerts_pending_count = loaded_alerts_collection.metadata('total') || collections.recipes.instance.findWhere({id: recipeId}).get('event_count'); // TODO, in the next update, `event_count` will be an object, and this will need to be `event_count.pending`
	...
	setLoadMoreButton: function(recipeId){
		...
		// TODO, stash this somewhere high up
		var page_size = 25;
		var $loadMore;
		...
		var currently_loaded_count = loaded_alerts_collection.models.length,
