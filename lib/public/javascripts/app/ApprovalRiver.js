app.ApprovalRiver = Backbone.View.extend({
	...
	bake: function(){
		...
		collections.recipes.instance.each(function(recipe){
			...
			this.pending_total += recipe.get('pending'); // TODO, make sure this is coming back as a number
		}, this);
	...
	getAssociatedAlertsForRecipe: {
			...
			if ((loaded_matches.length && mode != 'my-recipes') || (mode == 'my-recipes' && (loaded_matches.length >= page_limit || loaded_matches.length == pending))) {
				...
				options = { 
					...
					success: function(collection, response, options){
						console.log(options)
						that.getAssociatedAlertsForRecipe.cleanupFetch(collection, response, options);
					},
					error: function(err){
		...
		cleanupFetch: function(collection, results, options){
			...
			collections.all_alerts.instance.add(results);
