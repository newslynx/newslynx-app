	...
	sectionMode: {
		...
		detail: function(){
			...
			this.detail.loadModel(function(model) {
				...
				that.$subjectTagsContainer = that.$content.find('.tag-container[data-which="subject"] ul.tags')
				that.$impactTagsContainer  = that.$content.find('.tag-container[data-which="impact"] ul.tags')
				...
				// If we have subject tags, clear our placeholder 'No tags' and add them
				if (subject_tags_full.length){
					that.$subjectTagsContainer.html('');
