app.Articles = Backbone.View.extend({
	...
	setLoadMoreButton: function(){
		...
		if (diff > 0) {
			$articleCount.html('Showing ' + page_size + ' of ' + match_set_length + ' articles.');
		} else if (page_size >= total_length){
			$articleCount.html('Loaded all ' + total_length +  ' articles.');
		} else {
			$articleCount.html('Showing ' + match_set_length +  ' matching articles.');
		}
		// if (diff > 0) {
