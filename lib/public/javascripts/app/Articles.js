app.Articles = Backbone.View.extend({
	...
	initialize: function(){
		...
		// // Fetch or find the detailed data for that article and set the `models.article_detailed.instance` values to that data
	...
	sectionMode: {
		...
		detail: function(){
			...
			this.$content.html('');
			// Figure out which model we are loading
			var detail_model = this.detail.findModel();
	...
	detail: {
		findModel: function(cb){
			...
			if (collections.article_detailed.instance.length){
				detail_model = collections.article_detailed.instance.models;
			...
			// Have we already fetched that detail model from the server?
