app.helpers = {
	...
	drawer: {
		...
		determineBehavior: function(){
			var behavior = 'radio';
			if (models.section_mode.get('mode') == 'compare') behavior = 'checkbox';
			return behavior;
		},
		getAllUids: function(){
			var uids = [];
		...
		loadAllAlerts: function(){
			var section_mode = models.section_mode.get('mode'); // This should be `my-recipes`. Grab it from the mode instead of the string in case we change that name later
