app.Settings = Backbone.View.extend({
	...
	writeFormData: function($parentForm){
		var string_data = JSON.stringify(this.getCurrentFormDataFull($parentForm));
	...
	initColorPicker: function($el){
		...
		$el.find('.color-picker').each(function(){
			var group = $(this).parents('.js-inputs-container').attr('data-model'); // `'subject_tags'` or `'impact_tags'`
			$(this).spectrum({
				preferredFormat: "hex",
