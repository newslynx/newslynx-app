app.Settings = Backbone.View.extend({
	...
	events: {
		...
		'sync': 'saved',
		'focus .js-input-item': 'savePreviousValue',
		'keyup .js-input-item': 'trackChanges',
	...
	bakeInputActions: function(){
		$('.js-input-parent').each(function(){
			var $this = $(this),
					buttons_markup = templates.inputActionsFactory({});
	...
	trackChanges: function(e){
		var $inputItem 			= $(e.currentTarget),
				$inputParent = $inputItem.parents('.js-input-parent'),
				previous_val = $inputParent.attr('data-previous-value'),
		...
		this.displayInputActions($inputParent, show);
		return this;
	},

	displayInputActions: function($inputParent, show){
		$inputParent.attr('data-changed', show);
		return this;
	},
	...
	saveModel: function(e){
		var $saveBtn = $(e.currentTarget),
				...
				which_setting = $inputsContainer.attr('data-setting-name'),
				data_to_save = $inputParent.find('.js-input-item').val(),
				model_data = {
					name: which_setting,
					value: data_to_save
				};
		...
		this.displayInputActions($inputParent, false);
