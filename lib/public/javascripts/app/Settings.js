app.Settings = Backbone.View.extend({
	...
	render: function(){
		...
		var pw_setting_el = this.$content.find('.js-inputs-container[data-setting-name="password"]')[0];
		var pw_model = new models.setting.Model;
		var pw_setting_view = new views.SettingPassword({model: pw_model, el: pw_setting_el});
		this._subviews.push(pw_setting_view);
		collections.user_settings.instance.add(pw_model);
