app.Settings = Backbone.View.extend({
	...
	render: function(){
		...
		collections.user_settings.instance.each(function(userSetting){
			...
			var setting_view = new views.SettingSingle({model: userSetting, el: setting_el});
		...
		collections.settings.instance.each(function(orgSetting){
			...
			var setting_el = this.$content.find('.js-inputs-container[data-setting-name="'+name+'"]')[0];
			var setting_view = new views.SettingSingle({model: orgSetting, el: setting_el});
			this._subviews.push(setting_view);

		}, this);


		// For every colorpicker element, go ahead and instantiate it.
		// Maybe change this so that every color picker thing instantiates itself
