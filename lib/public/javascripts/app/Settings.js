app.Settings = Backbone.View.extend({
	...
	bakeInputActions: function(){
		$('.js-input-parent').each(function(){
			var $this = $(this),
					buttons_markup = templates.inputActionsFactory({}), // Our template accepts no data options
					saved_setting = $this.attr('data-saved-setting'),
					current_val = $this.find('.js-input-item').val(),
					show = current_val != saved_setting;

			// This determines, via css, whether the cancel/save buttons container appears
			// if someon refreshes the settings pages and has inputted new settings but not saved them
			...
			$this.attr('data-changed', show.toString());

			$this.append(buttons_markup);
		});
		return this;
	},
	...
	trackChanges: function(e){
		var $inputItem 	= $(e.currentTarget),
				$inputParent = $inputItem.parents('.js-input-parent'),
				previous_val = $inputParent.attr('data-saved-setting'),
				current_val = $inputItem.val(),
				show;
		...
		this.setDataChanged($inputParent, show);
		return this;
	},

	setDataChanged: function($inputParent, show){
		$inputParent.attr('data-changed', show.toString());
		return this;
	},

	revertToPreviousSettingVal: function(e){
		var $inputParent = $(e.currentTarget).parents('.js-input-parent'),
				previous_val = $inputParent.attr('data-saved-setting');

		$inputParent.attr('data-changed', 'false').find('.js-input-item').val(previous_val);
	...
	saveModel: function(e){
		...
		model_to_save.save(null, {
			success: function(model, response, options){
				console.log('success');
				that.setDataChanged($inputParent, false);
				$inputParent.attr('data-saved-setting', data_value_to_save);
			},
