...
router.get('/exports', function(req, res) {
  ...
  content_item_ids_list.forEach(function(contentItemId){
    ...
    qEvents.defer(utils.auth.sendToApi(req, res), '/api/_VERSION/events?per_page=100&status=approved&sous_chefs=-twitter-search-content-item-links-to-event&content_item_ids=' + contentItemId)
  })
  ...
  qMeta.await(function(err, articleLists, timeseriesLists, tweetsLists, eventsLists){
    ...
    csvs[timestamp+'_article_'+content_item_ids_list.join('-')] = articleLists;
    content_item_ids_list.forEach(function(id, index){
      csvs[timestamp+'_article_'+id+'_timeseries'] = timeseriesLists[index];
      csvs[timestamp+'_article_'+id+'_tweets'] = tweetsLists[index]
      csvs[timestamp+'_article_'+id+'_events'] = eventsLists[index]
    })
