...
router.get('/exports', function(req, res) {
  ...
  qMeta.defer(qArticleInfo.awaitAll);
  qMeta.defer(export_helpers.timeseries, qTimeseries.awaitAll, now);
  qMeta.defer(export_helpers.events.qTweets.awaitAll);
  qMeta.defer(export_helpers.events.qEvents.awaitAll);

  // Await for our queues of queues
  qMeta.await(function(err, articleLists, timeseriesLists, tweetInfo, eventInfo){
    if (err) {
      debug.log(err)
    }
    var status_code = (err) ? 500 : 200;

    // var events_by_id = export_helpers.segregateById(eventInfo[0].events, content_item_ids_list);
    // var tweets_by_id = export_helpers.segregateById(tweetInfo[0].events, content_item_ids_list);

    // Add each list element to csv by id
    content_item_ids_list.forEach(function(id, index){
      csvs[timestamp+'_article_'+id+'_timeseries'] = timeseriesLists[index];
      // csvs[timestamp+'_article_'+id+'_tweets'] = export_helpers.events.call(delimiters, tweets_by_id[index], id).map(export_helpers.addExportedDate(now));
      // csvs[timestamp+'_article_'+id+'_events'] = export_helpers.events.call(delimiters, events_by_id[index], id).map(export_helpers.addExportedDate(now));
    })
