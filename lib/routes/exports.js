...
router.get('/exports', function(req, res) {
  ...
  qMeta.await(function(err, articleLists, timeseriesLists, tweetsLists, eventsLists){
     ...
     * For adding them all in one csv with an article id column, if that exists
      csvs[timestamp+'_article_'+id+'_timeseries'] = _.flatten(timeseriesLists)
      csvs[timestamp+'_article_'+id+'_tweets'] = _.flatten(tweetsLists)
      csvs[timestamp+'_article_'+id+'_events'] = _.flatten(eventsLists)
    */

    content_item_ids_list.forEach(function(id, index){
      csvs[timestamp+'_article_timeseries_' + ids_dashed] = timeseriesLists[index]
      csvs[timestamp+'_article_tweets_' + ids_dashed] = tweetsLists[index]
      csvs[timestamp+'_article_events_' + ids_dashed] = eventsLists[index]
    })
