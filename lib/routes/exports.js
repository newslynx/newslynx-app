...
router.get('/exports', function(req, res) {
  ...
  qMeta.await(function(err, timeseriesLists, tweetInfo, eventInfo){
    ...
    content_item_ids_list.forEach(function(id, index){
      ...
      csvs[timestamp+'_article_'+id+'_tweets'] = utils.transform.exportHelpers.events.call(delimiters, tweets_by_id[index], id);
      csvs[timestamp+'_article_'+id+'_events'] = utils.transform.exportHelpers.events.call(delimiters, events_by_id[index], id);
    })
