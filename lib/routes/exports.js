...
router.get('/exports', function(req, res) {
  ...
  qEvents.defer(utils.auth.sendToApi(req, res), '/api/_VERSION/events?per_page=100&sous_chefs=-twitter-search-content-item-links-to-event&content_item_ids=' + content_item_ids_str)
  ...
  qMeta.await(function(err, timeseriesLists, tweetInfo, eventInfo){
    ...
    var events_by_id = utils.transform.segregateById(eventInfo[0].events, content_item_ids_list);
    var tweets_by_id = utils.transform.segregateById(tweetInfo[0].events, content_item_ids_list);
    ...
    content_item_ids_list.forEach(function(id, index){
      ...
      csvs[timestamp+'_article_'+id+'_tweets'] = tweets_by_id[index];
      csvs[timestamp+'_article_'+id+'_events'] = events_by_id[index];
    })
