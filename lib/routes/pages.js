...
function logNullResponse(err, results){
  ...
  results.forEach(function(result, index){
    if (!result || result.status_code >= 400) {
      console.log(chalk.red('ERROR, '+result.status_code+' status code at index:'), index, chalk.red('\nWith object'), result);
    } 
  });
...
router.get('/settings', function(req, res) {
  ...
  if (auth_success === 'true') {
    recipe_urls = auth_success_routines[auth_service].map(function(slug){
