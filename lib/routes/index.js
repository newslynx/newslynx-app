...
router.get('/approval-river', function(req, res) {
	...
	queue()
		...
		.defer(sendToApi(req, res), '/api/_VERSION/events?provenance=manual&per_page=1')
			.await(function(err, impactTagsInfo, sousChefsInfo, recipesInfo, eventsInfo, defaultEventSchema, alertFieldsSchema, timezoneSettingInfo, manualEventsInfo){
				// logNullResponse(err, [impactTagsInfo, sousChefsInfo, recipesInfo, eventsInfo, defaultEventSchema, alertFieldsSchema, timezoneSettingInfo]);
