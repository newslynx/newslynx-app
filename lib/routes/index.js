...
router.get('/approval-river', function(req, res) {
	...
	async.map([
	  ...
	  'lib/public/data/common-data-models/recipe-models.json', 
	  'lib/public/data/account-data/account-recipes.json', 
	  'lib/public/data/account-data/account-alerts.json'], 
...
router.get('/settings', function(req, res) {
	...
	info.title = 'Settings';
	info.page = sanitize(info.title);
	async.map(['/api/organization/settings'], sendToApi(req,res), function(err, results){
		info.org = results[0];
