...
function grantAccess(service){
	return function(req, res) {
		res.redirect(settings.api + '/api/'+settings.config.api_version+'/auth/'+service+'?apikey=' + req.session.apikey  + '&org=' + req.session.org_id + '&redirect_uri=' + req.headers.referer);
	}
}
...
router.get('/settings', function(req, res) {
	...
	queue()
		...
		.defer(sendToApi(req,res), '/api/_VERSION/auth/google-analytics')
		.defer(sendToApi(req,res), '/api/_VERSION/auth/twitter')
		.defer(sendToApi(req,res), '/api/_VERSION/auth/facebook')
			.await(function(err, userInformation, orgSettings, tagsObj, recipesList, googleAnalyticsAuth, twitterAuth, facebookAuth){
				...
				info.userSettingsList = utils.transform.turnUserDictTolist(userInformation, ['email']);
				...
				// debug.log(googleAnalyticsAuth)
				// debug.log(twitterAuth)
				debug.log(facebookAuth)
				info.org.google_analytics_authed = false;
				info.org.twitter_authed = false;
				info.org.facebook_authed = false;
...
router.get('/approval-river', function(req, res) {
	...
	queue()
		...
		.defer(sendToApi(req, res), '/api/_VERSION/organization/settings') // Replace this with timezone endpoint
...
// Authentications
router.get('/auth/google-analytics', grantAccess('google-analytics') );
router.get('/auth/twitter', grantAccess('twitter') );
router.get('/auth/facebook', grantAccess('facebook') );
