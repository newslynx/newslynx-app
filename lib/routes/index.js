	...
	queue()
		...
		.defer(sendToApi(req, res), '/api/_VERSION/metrics?sort=name')
			.await(function(err,    tagCategories, tagLevels, tagAttributeColors, tagsInfo, contentInfo, eventSchema, timezoneSettingInfo, articleSchema, metricsInfo){
				...
				var dimensions_sort_by,
						dimensions_sort_ascending;

				if (/\./.test(default_sort_bys)){
					dimensions_sort_by = default_sort_bys.split('.')[1];
				} else if (/^-/.test(default_sort_bys)){
					dimensions_sort_by = default_sort_bys.replace('-','');
				} else {
					dimensions_sort_by = default_sort_bys
				}

				if (/^-/.test(default_sort_bys)){
					dimensions_sort_ascending = false;
				} else {
					dimensions_sort_ascending = true;
				}
				...
				info.articleSummariesInfo = {
					...
					sort_by: default_sort_bys // This is the 
				}
				// Timezone
				info.timezone = timezoneSettingInfo.value;
				...
				info.dimensionsInfo = {
					...
					sort_by: dimensions_sort_by,
					sort_ascending: dimensions_sort_ascending
				};
