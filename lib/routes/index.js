...
router.get('/approval-river', function(req, res) {
	...
	queue()
		...
		.defer(sendToApi(req, res), '/api/_VERSION/recipes?status=pending')
		.defer(sendToApi(req, res), '/api/_VERSION/events?include_raw=true&count=25')
		...
		.defer(sendToApi(req, res), '/api/_VERSION/settings/timezone')
			.await(function(err, impactTags, sousChefsInfo, recipesInfo, eventsInfo, defaultEventSchema, alertFieldsSchema, articlesList, timezoneSettingInfo){
				logNullResponse(err, [impactTags, sousChefsInfo, recipesInfo, eventsInfo, defaultEventSchema, alertFieldsSchema, articlesList, timezoneSettingInfo]);
