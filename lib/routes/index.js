...
router.get('/approval-river', function(req, res) {
	...
	queue()
		...
		.defer(sendToApi(req, res), '/api/alerts?include_raw=true')
		...
		.defer(sendToApi(req, res), '/api/articles?fields=id,title')
			.awaitAll(function(err, results){
				// debug.log(err, typeof results, results)
				// Data
		...
		queue()
			...
			.awaitAll(function(err, results){
				...
				if (id){
					...
					res.render('login', {title: 'Login', page: 'login', page: 'login', msg: 'Article lookup failed! NewsLynx doesn\'t have a page for that article.'});
				}
			})
...
router.get('/auth/google/revoke', function(req, res) {
	var apiRequester = sendToApi(req,res),
			path = settings.api + '/auth/google/revoke?apikey=' + req.session.apikey;

	apiRequester(path, function(response){
...
router.get('/auth/twitter/revoke', function(req, res) {
	// res.redirect(settings.api + '/auth/twitter/revoke?apikey=' + req.session.apikey);
	var apiRequester = sendToApi(req,res),
			path = settings.api + '/auth/twitter/revoke?apikey=' + req.session.apikey;
