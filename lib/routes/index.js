...
router.get('/approval-river', function(req, res) {
	...
	queue()
		...
		.defer(sendToApi(req, res), '/api/_VERSION/events?status=pending&per_page=25')
		...
		.defer(sendToApi(req, res), '/api/_VERSION/events?provenance=manual&per_page=0')
			.await(function(err, impactTagsInfo, sousChefsInfo, recipesInfo, eventsInfo, defaultEventSchema, alertFieldsSchema, articlesList, timezoneSettingInfo, manualEventsInfo){
				// logNullResponse(err, [impactTagsInfo, sousChefsInfo, recipesInfo, eventsInfo, defaultEventSchema, alertFieldsSchema, articlesList, timezoneSettingInfo]);
				...
				// console.log(impactTagsInfo)
				// Add full tags to the default event schema
				defaultEventSchema.tag_ids.options = impactTagsInfo.tags;
				...
				info.articleSkeletons = articlesList;
				// Timezone
