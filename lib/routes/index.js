...
router.get('/approval-river', function(req, res) {
	...
	queue()
		...
		.defer(sendToApi(req, res), '/api/_VERSION/events?status=pending&per_page=25&provenance=recipe')
		...
		.defer(sendToApi(req, res), '/api/_VERSION/settings/timezone')
			.await(function(err, impactTagsInfo, sousChefsInfo, recipesInfo, eventsInfo, defaultEventSchema, alertFieldsSchema, articlesList, timezoneSettingInfo){
				...
				defaultEventSchema.impact_tags.options = impactTagsInfo.tags;
