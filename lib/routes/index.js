...
router.get('/settings', function(req, res) {
	...
	queue()
		...
		.defer(sendToApi(req,res), '/api/_VERSION/orgs')
			.await(function(err, userInformation, orgSettings, tagsObj, recipesList, orgInfo){
				...
				var auths = orgInfo[0].auths;

				info.org.google_analytics_authed = utils.transform.hasObject(auths, {name: 'google-analytics'});
				info.org.twitter_authed =  utils.transform.hasObject(auths, {name: 'twitter'});
				info.org.facebook_authed =  utils.transform.hasObject(auths, {name: 'facebook'});
