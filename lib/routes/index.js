	...
	queue()
		.defer(sendToApi(req,res), '/api/organization/settings')
			.awaitAll(function(err, results){
				...
				var info_gzipped = utils.transform.gzip(info);
				res.render('settings', {info: info_gzipped});
			});
});
	...
	queue()
		...
		.defer(sendToApi(req, res), '/api/organization/settings') // Replace this with timeone endpoint
			.awaitAll(function(err, results){
				...
				var info_gzipped = utils.transform.gzip(info);
				res.render('approval-river', {info: info_gzipped});
			});
});
	...
	queue()
		...
		.defer(readFileAsJson, 'lib/public/data/common-data-models/article-schema.json')
			.awaitAll(function(err, results){
				...
				var info_gzipped = utils.transform.gzip(info);
				res.render('articles', {info: info_gzipped});
			});
});
