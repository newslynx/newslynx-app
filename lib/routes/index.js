...
router.get('/settings', function(req, res) {
	...
	// Fetch data
	queue()
		.defer(sendToApi(req,res), '/api/:version/me')
		.defer(sendToApi(req,res), '/api/:version/orgs/'+req.session.org_id+'/settings')
		.defer(sendToApi(req,res), '/api/:version/tags')
			.await(function(err, userInformation, orgSettings, tagsObj){
...
router.get('/approval-river', function(req, res) {
	...
	// Fetch data
	queue()
		.defer(sendToApi(req, res), '/api/:version/tags/impact')
		.defer(readFileAsJson, 'lib/public/data/common-data-models/recipe-schemas.json')
		.defer(sendToApi(req, res), '/api/:version/recipes')
		.defer(sendToApi(req, res), '/api/:version/alerts?include_raw=true&count=25')
		...
		.defer(sendToApi(req, res), '/api/:version/articles?fields=id,title')
		.defer(sendToApi(req, res), '/api/:version/organization/settings') // Replace this with timezone endpoint
...
router.get('/articles', function(req, res) {
	...
	// Fetch data
	queue()
		.defer(sendToApi(req, res), '/api/:version/tags/impact/attributes/categories')
		.defer(sendToApi(req, res), '/api/:version/tags/impact/attributes/levels')
		.defer(sendToApi(req, res), '/api/:version/tags/subject')
		.defer(sendToApi(req, res), '/api/:version/tags/impact')
		.defer(sendToApi(req, res), '/api/:version/articles')
		.defer(readFileAsJson, 'lib/public/data/common-data-models/event-schema.json')
		.defer(readFileAsJson, 'lib/public/data/common-data-models/event-schema-alert-data.json')
		.defer(sendToApi(req, res), '/api/:version/articles/comparisons') 
		.defer(readFileAsJson, 'lib/public/data/common-data-models/tag-attribute-colors.json')
		.defer(sendToApi(req, res), '/api/:version/organization/settings') // Replace this with timeone endpoint
...
router.get('/articles/lookup', function(req, res){
	...
	if (url){
		queue()
			.defer(sendToApi(req, res), '/api/:version/articles/lookup?url='+url)
			.awaitAll(function(err, results){
				var id = results[0].id,
						info;
