...
router.post('/auth/_VERSION/login', function(req, res){
	...
	req.session.redirect_page = req.session.redirect_page || '/'
...
router.post('/setup', function(req, res){
	...
	if (url) {
		...
		settings.save(settings.config, function(err){
			if (err) {
				console.log('Error saving api_url to config', err)
			}
			info = {title: 'Login', page: 'login', page: 'login', msg: JSON.stringify(err)};
			res.render('login', {info: info})
		});
