...
function grantAccess(service){
  return function(req, res) {
    res.redirect(settings.config.api_url + '/api/' + settings.config.api_version + '/auths/' + service + '/grant?apikey=' + req.session.apikey  + '&org=' + req.session.org_id + '&redirect_uri=' + req.headers.referer + '?service=' + service);
  }
}
